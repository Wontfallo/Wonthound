// =============================================================================
// WontHound-CYD UART Serial Terminal
...
static void drawBaudSelector() {
    ...
    // Rounded rect border
    tft.drawRoundRect(baudX, baudY, baudW, 40, 6, WONTHOUND_MAGENTA);
...
static void drawPinSelector() {
    ...
    // P1 DUPLEX button
    if (selectedPin == UART_PIN_P1) {
        tft.fillRoundRect(12, pinY + 2, btnW, 30, 6, WONTHOUND_MAGENTA);
        tft.setTextColor(TFT_BLACK);
    } else {
        tft.drawRoundRect(12, pinY + 2, btnW, 30, 6, WONTHOUND_GUNMETAL);
    ...
    if (selectedPin == UART_PIN_SPEAKER) {
        tft.fillRoundRect(spkX, pinY + 2, btnW, 30, 6, WONTHOUND_MAGENTA);
        tft.setTextColor(TFT_BLACK);
    } else {
        tft.drawRoundRect(spkX, pinY + 2, btnW, 30, 6, WONTHOUND_GUNMETAL);
        tft.setTextColor(WONTHOUND_GUNMETAL);
    }
    tft.setTextSize(1);
...
static void drawTermIconBar() {
    ...
    tft.drawBitmap(40, ICON_BAR_Y, monPaused ? bitmap_icon_eye2 : bitmap_icon_eye, ICON_SIZE, ICON_SIZE, WONTHOUND_MAGENTA);
    tft.drawBitmap(70, ICON_BAR_Y, bitmap_icon_recycle, ICON_SIZE, ICON_SIZE, WONTHOUND_MAGENTA);
...
void serialMonitorScreen() {
    ...
    while (!exitRequested) {
        ...
        if (isTermBackTapped()) {
            ...
            monPaused = !monPaused;
            // Update pause icon
            tft.fillRect(40, ICON_BAR_Y, ICON_SIZE, ICON_SIZE, WONTHOUND_DARK);
            tft.drawBitmap(40, ICON_BAR_Y, monPaused ? bitmap_icon_eye2 : bitmap_icon_eye,
                           ICON_SIZE, ICON_SIZE, WONTHOUND_MAGENTA);
            if (!monPaused) {
