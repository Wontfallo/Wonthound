...
void TAMC_GT911::reset() {
  // WontHound patch: skip INT pin operations when pin is 0xFF (-1 as uint8_t)
  // ESP32-3248S035C has R25 not populated — INT line disconnected from GPIO 21
  bool hasIntPin = (pinInt != GT911_PIN_NONE);
  ...
  Serial.printf("[GT911] Config version=0x%02X, maxX=%d, maxY=%d, touchNum=%d\n",
                configBuf[0],
                configBuf[1] | (configBuf[2] << 8),
                configBuf[3] | (configBuf[4] << 8),
                configBuf[5]);
  Serial.printf("[GT911] ModSwitch1=0x%02X filter=0x%02X touchLevel=%d releaseLevel=%d\n",
                configBuf[GT911_MODULE_SWITCH_1 - GT911_CONFIG_START],
                configBuf[GT911_FILTER - GT911_CONFIG_START],
                configBuf[GT911_SCREEN_TOUCH_LEVEL - GT911_CONFIG_START],
                configBuf[GT911_SCREEN_RELEASE_LEVEL - GT911_CONFIG_START]);
