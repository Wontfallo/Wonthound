// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD SubGHz Attack Modules Implementation
...
// Initialize color palette - WONTHOUND CYBERPUNK (matches packet monitor)
...
static void updateDisplay() {
    ...
    tft.fillRect(0, CONTENT_Y_START, SCREEN_WIDTH, 14, WONTHOUND_BLACK);
    tft.setTextSize(1);
    ...
    if (autoScanEnabled) {
        tft.setTextColor(autoScanPaused ? WONTHOUND_HOTPINK : WONTHOUND_BRIGHT);
    ...
    tft.setTextColor(rssi > -60 ? WONTHOUND_HOTPINK : WONTHOUND_VIOLET);
    tft.print(rssi);
}
...
static void drawUI() {
    ...
    tft.drawLine(0, CONTENT_Y_START + 15, SCREEN_WIDTH, CONTENT_Y_START + 15, WONTHOUND_HOTPINK);
...
static void drawSignalCaptured() {
    ...
    // Flash effect
    tft.fillRect(panelX, panelY, panelW, panelH, WONTHOUND_HOTPINK);
    delay(50);
    tft.fillRect(panelX, panelY, panelW, panelH, WONTHOUND_DARK);

    // Draw capture panel with border
    tft.drawRect(panelX, panelY, panelW, panelH, WONTHOUND_HOTPINK);
    tft.drawRect(panelX + 1, panelY + 1, panelW - 2, panelH - 2, WONTHOUND_MAGENTA);
...
void sendSignal(unsigned long value, int bitLength, int protocol) {
    ...
    tft.fillRect(0, SCALE_Y(100), SCREEN_WIDTH, SCALE_Y(80), WONTHOUND_BLACK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawJammerDisplay() {
    ...
    if (!jamming) {
        ...
        if (!hasHeat) {
            ...
            tft.drawRect(SJ_GRAPH_X - 1, SJ_GRAPH_Y - 1, SJ_GRAPH_WIDTH + 2, SJ_GRAPH_HEIGHT + 2, WONTHOUND_MAGENTA);
...
void setup() {
    ...
    if (ELECHOUSE_cc1101.getCC1101()) {
        ...
        tft.setTextColor(WONTHOUND_HOTPINK);
        tft.setTextSize(2);
        drawCenteredText(100, "CC1101 NOT FOUND", WONTHOUND_HOTPINK, 2);
        tft.setTextSize(1);
        drawCenteredText(130, "Check wiring:", WONTHOUND_MAGENTA, 1);
        drawCenteredText(145, "CS=27 GDO0=22 GDO2=35", WONTHOUND_MAGENTA, 1);
        initialized = false;
        return;
    }
...
static void drawEqualizer() {
    ...
    tft.fillRect(10, EQ_Y, CONTENT_INNER_W, 20, WONTHOUND_BLACK);
    tft.drawRoundRect(10, EQ_Y, CONTENT_INNER_W, 20, 4, WONTHOUND_MAGENTA);
...
static void drawGradientProgressBar(int x, int y, int w, int h, float progress) {
    // Border
    tft.drawRoundRect(x - 1, y - 1, w + 2, h + 2, 3, WONTHOUND_MAGENTA);
...
static void drawMainUI() {
    ...
    tft.drawRoundRect(10, SCALE_Y(60), CONTENT_INNER_W, SCALE_Y(55), 8, WONTHOUND_VIOLET);
    tft.drawRoundRect(11, SCALE_Y(61), CONTENT_INNER_W - 2, SCALE_Y(53), 7, WONTHOUND_GUNMETAL);

    // Protocol name - big and centered
    tft.fillRect(15, SCALE_Y(65), CONTENT_INNER_W - 10, SCALE_Y(22), TFT_BLACK);
    drawCenteredFreeFont(SCALE_Y(84), proto.name, WONTHOUND_MAGENTA, &FreeMonoBold12pt7b);
    ...
    drawCenteredFreeFont(SCALE_Y(105), freqBuf, WONTHOUND_BRIGHT, &FreeMono9pt7b);
...
static void updateDisplay() {
    ...
    if (running) {
        if (paused) {
            drawCenteredFreeFont(SCALE_Y(155), "PAUSED", WONTHOUND_VIOLET, &FreeMonoBold12pt7b);
        } else {
        ...
        drawCenteredFreeFont(SCALE_Y(155), "READY", WONTHOUND_BRIGHT, &FreeMonoBold12pt7b);
        tft.setFreeFont(NULL);
        tft.setTextSize(1);
        tft.setTextColor(WONTHOUND_VIOLET, TFT_BLACK);
...
// ═══════════════════════════════════════════════════════════════════════════
// HEAT MAP PALETTE — WONTHOUND CYBERPUNK
...
static void drawSpectrumBars() {
    for (int ch = 0; ch < frequencyCount; ch++) {
        ...
        if (peakSeg > litSegs && peakSeg > 0) {
            ...
            tft.fillRect(barX, peakY, BAR_WIDTH, SEG_HEIGHT, WONTHOUND_HOTPINK);
        }
...
static void drawLineGraph() {
    ...
    for (int x = 1; x < WF_WIDTH; x++) {
        ...
        uint16_t color;
        if (avgLevel <= 0) {
            color = WONTHOUND_GUNMETAL;  // Dim baseline when no signal
        } else {
    ...
    tft.drawFastHLine(WF_X, AXIS_Y, WF_WIDTH, WONTHOUND_MAGENTA);
    tft.drawFastVLine(WF_X - 1, LG_Y, LG_HEIGHT + 1, WONTHOUND_MAGENTA);
...
static void drawStaticElements() {
    ...
    tft.drawFastHLine(0, SEP_Y, SCREEN_WIDTH, WONTHOUND_HOTPINK);

    // Line graph axes
    tft.drawFastHLine(WF_X, AXIS_Y, WF_WIDTH, WONTHOUND_MAGENTA);            // X axis
    tft.drawFastVLine(WF_X - 1, LG_Y, LG_HEIGHT + 1, WONTHOUND_MAGENTA);    // Y axis
