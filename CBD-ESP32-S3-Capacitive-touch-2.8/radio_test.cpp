...
// Forward declarations for functions defined in WontHound-CYD.ino
...
// Draw a WontHound gradient progress bar (MAGENTA → HOTPINK)
...
static void drawGradientBar(int y, int h, float progress) {
    ...
    // Background (dark)
    tft.fillRect(barX, y, barW, h, WONTHOUND_DARK);

    // Extract RGB565 components from actual theme colors
    uint8_t r1 = (WONTHOUND_MAGENTA >> 11) & 0x1F;
    uint8_t g1 = (WONTHOUND_MAGENTA >> 5)  & 0x3F;
    uint8_t b1 =  WONTHOUND_MAGENTA        & 0x1F;
    uint8_t r2 = (WONTHOUND_HOTPINK >> 11) & 0x1F;
    uint8_t g2 = (WONTHOUND_HOTPINK >> 5)  & 0x3F;
    uint8_t b2 =  WONTHOUND_HOTPINK        & 0x1F;
    ...
    // Border
    tft.drawRect(barX - 1, y - 1, barW + 2, h + 2, WONTHOUND_MAGENTA);
}
...
static void runGpsTest(int statusY, int hintY) {
    ...
    drawStatusLine(hintY, "Initializing GPS...", WONTHOUND_GUNMETAL);
    ...
    drawStatusLine(hintY, "Sampling GPS data...", WONTHOUND_GUNMETAL);
...
static void drawNrf24Diagram() {
    ...
    // Left box — ESP32
    tft.drawRect(DIAG_LEFT_X, boxY, DIAG_LEFT_W, boxH, WONTHOUND_MAGENTA);
    tft.drawRect(DIAG_LEFT_X + 1, boxY + 1, DIAG_LEFT_W - 2, boxH - 2, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK, TFT_BLACK);
    tft.setCursor(DIAG_LEFT_X + 25, boxY + 4);
    tft.print("ESP32");

    // Right box — NRF24L01+
    tft.drawRect(DIAG_RIGHT_X, boxY, DIAG_RIGHT_W, boxH, WONTHOUND_MAGENTA);
    tft.drawRect(DIAG_RIGHT_X + 1, boxY + 1, DIAG_RIGHT_W - 2, boxH - 2, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK, TFT_BLACK);
    ...
    drawPinTrace(py, csnLabel, "CSN", WONTHOUND_MAGENTA,  false); }  py += pinSpace;
    { char ceLabel[8]; snprintf(ceLabel, sizeof(ceLabel), "IO%d", NRF24_CE);
    drawPinTrace(py, ceLabel, "CE",   WONTHOUND_MAGENTA,  false); }  py += pinSpace;
    drawPinTrace(py, "IO17",  "IRQ",  WONTHOUND_GUNMETAL, true);
...
static void drawCC1101Diagram() {
    ...
    drawPinTrace(py, csLabel,  "CS",      WONTHOUND_MAGENTA, false);  py += pinSpace;
    drawPinTrace(py, "IO18",  "SCK",     TFT_CYAN,          false);  py += pinSpace;
    drawPinTrace(py, "IO23",  "MOSI",    TFT_CYAN,          false);  py += pinSpace;
    drawPinTrace(py, "IO19",  "MISO",    TFT_CYAN,          false);  py += pinSpace;
    drawPinTrace(py, "IO22",  "GDO0 TX", WONTHOUND_HOTPINK, false);  py += pinSpace;
...
static void drawPN532Diagram() {
    ...
    drawPinTrace(py, csLabel, "SS",   WONTHOUND_MAGENTA,  false); }  py += pinSpace;
    drawPinTrace(py, "IO18",  "SCK",  TFT_CYAN,           false);  py += pinSpace;
    drawPinTrace(py, "IO23",  "MOSI", TFT_CYAN,           false);  py += pinSpace;
...
static void drawMainScreen() {
    ...
    drawRadioButton(RT_NRF_BTN_Y, RT_NRF_BTN_H, "[ NRF24L01+ ]", WONTHOUND_MAGENTA);
    drawStatusLine(RT_NRF_STATUS_Y, "Status: --", WONTHOUND_GUNMETAL);

    // CC1101 button and status
    drawRadioButton(RT_CC_BTN_Y, RT_CC_BTN_H, "[ CC1101 ]", WONTHOUND_MAGENTA);
    drawStatusLine(RT_CC_STATUS_Y, "Status: --", WONTHOUND_GUNMETAL);

    // GPS button and status
#if CYD_HAS_GPS
    drawRadioButton(RT_GPS_BTN_Y, RT_GPS_BTN_H, "[ GPS ]", WONTHOUND_MAGENTA);
    drawStatusLine(RT_GPS_STATUS_Y, "Status: --", WONTHOUND_GUNMETAL);
#else
    drawRadioButton(RT_GPS_BTN_Y, RT_GPS_BTN_H, "[ GPS N/A ]", WONTHOUND_GUNMETAL);
    drawStatusLine(RT_GPS_STATUS_Y, "GPS disabled in config", WONTHOUND_GUNMETAL);
#endif

    // Wiring reference button
    drawRadioButton(RT_WIRE_BTN_Y, RT_WIRE_BTN_H, "[ WIRING ]", WONTHOUND_HOTPINK);

    // Hint
    drawCenteredText(RT_HINT_Y, "Tap radio to test", WONTHOUND_HOTPINK, 1);
}
...
void radioTestScreen() {
    ...
    while (!exitRequested) {
        ...
        if (isTouchInArea(RT_BTN_X, RT_NRF_BTN_Y, RT_BTN_W, RT_NRF_BTN_H)) {
            ...
            drawCenteredText(RT_HINT_Y, "Tap again to re-test", WONTHOUND_GUNMETAL, 1);
