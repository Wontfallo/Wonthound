// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Unified Loot Manager
...
static void drawLMIconBar() {
    tft.fillRect(0, ICON_BAR_Y, tft.width(), ICON_BAR_H, WONTHOUND_DARK);
...
static void drawHub() {
    ...
    for (int i = 0; i < LM_HUB_ROWS; i++) {
        ...
        if (i % 2 == 0) {
            tft.fillRect(2, y, SCREEN_WIDTH - 4, LM_HUB_ROW_H - 2, WONTHOUND_DARK);
        }

        // Icon
        tft.drawBitmap(8, y + 10, hubIcons[i], 16, 16, WONTHOUND_MAGENTA);
...
static void drawWDListHeader() {
    ...
    tft.drawFastHLine(4, LM_WD_LIST_Y - 2, SCREEN_WIDTH - 8, WONTHOUND_VIOLET);
}

static void drawWDFileRow(int screenRow, int fileIdx, bool selected) {
    int y = LM_WD_LIST_Y + screenRow * LM_WD_ROW_H;
    tft.fillRect(2, y, SCREEN_WIDTH - 4, LM_WD_ROW_H - 1, selected ? WONTHOUND_VIOLET : TFT_BLACK);
    ...
    tft.setFreeFont(NULL);
    tft.setTextSize(1);
    tft.setTextColor(selected ? TFT_WHITE : WONTHOUND_MAGENTA);

    // Display name: strip "wonthound_" prefix and ".csv" suffix for cleaner look
    char displayName[24];
    const char* src = f.name;
    if (strncasecmp(src, "wonthound_", 10) == 0) src += 10;
...
static void drawWDEmpty() {
    ...
    drawGlitchText(55, "WARDRIVING", &Nosifer_Regular10pt7b);
    tft.drawLine(0, 58, tft.width(), 58, WONTHOUND_HOTPINK);
    drawGlitchStatus(120, "NO FILES", WONTHOUND_HOTPINK);
    drawCenteredText(160, "No CSVs in /wardriving/", WONTHOUND_GUNMETAL, 1);
    drawCenteredText(180, "Run Wardriving first", WONTHOUND_GUNMETAL, 1);
}
...
static void drawWDDetail() {
    ...
    // Stats box
    tft.drawRoundRect(10, 64, CONTENT_INNER_W, 120, 4, WONTHOUND_VIOLET);
    tft.drawRoundRect(11, 65, CONTENT_INNER_W - 2, 118, 4, WONTHOUND_VIOLET);
    ...
    // Open networks
    tft.setTextColor(wdStats.openCount > 0 ? WONTHOUND_GREEN : WONTHOUND_GUNMETAL);
    ...
    // SERIAL button
    tft.drawRoundRect(btn1X, btnY, btnW, 32, 4, WONTHOUND_VIOLET);
    tft.drawRoundRect(btn1X + 1, btnY + 1, btnW - 2, 30, 4, WONTHOUND_VIOLET);
    tft.setTextDatum(TC_DATUM);
    tft.setTextColor(WONTHOUND_VIOLET);
    tft.drawString("SERIAL", btn1X + btnW / 2, btnY + 10);

    // DELETE button
    tft.drawRoundRect(btn2X, btnY, btnW, 32, 4, WONTHOUND_HOTPINK);
    tft.drawRoundRect(btn2X + 1, btnY + 1, btnW - 2, 30, 4, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawWDDeleteConfirm() {
    ...
    tft.drawRoundRect(20, 120, SCREEN_WIDTH - 40, 80, 4, WONTHOUND_HOTPINK);
    tft.drawRoundRect(21, 121, SCREEN_WIDTH - 42, 78, 4, WONTHOUND_HOTPINK);
    ...
    // YES button
    tft.drawRoundRect(35, 168, 70, 24, 4, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.drawString("YES", 70, 172);

    // NO button
    tft.drawRoundRect(135, 168, 70, 24, 4, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void handleWDDetailTouch(uint16_t tx, uint16_t ty) {
    ...
    if (tx >= btn1X && tx <= btn1X + btnW && ty >= btnY && ty <= btnY + 32) {
        ...
        // Flash "SENT!" feedback
        tft.fillRoundRect(btn1X + 1, btnY + 1, btnW - 2, 30, 4, WONTHOUND_VIOLET);
        tft.setTextDatum(TC_DATUM);
...
static uint16_t colorForLine(const char* line) {
    if (strstr(line, "[CRACKED]"))  return WONTHOUND_GREEN;
    if (strstr(line, "[OPEN]"))     return WONTHOUND_HOTPINK;
    if (strstr(line, "[LOCKED]"))   return WONTHOUND_GUNMETAL;
    if (strstr(line, "[PSK]"))      return WONTHOUND_HOTPINK;
    if (strstr(line, "rtsp://"))    return WONTHOUND_VIOLET;
    if (strstr(line, "http://"))    return WONTHOUND_VIOLET;
    if (strstr(line, "https://"))   return WONTHOUND_VIOLET;
    if (strstr(line, "mqtt://"))    return WONTHOUND_VIOLET;
    if (strstr(line, "telnet"))     return WONTHOUND_VIOLET;
    if (strstr(line, "Creds:"))     return WONTHOUND_GREEN;
    if (strstr(line, "==="))        return WONTHOUND_MAGENTA;
    if (line[0] == '|')            return WONTHOUND_MAGENTA;
    return WONTHOUND_CYAN;
}
...
void setup() {
    ...
    if (!sdMounted) {
        ...
        drawGlitchText(55, "LOOT", &Nosifer_Regular10pt7b);
        tft.drawLine(0, 58, tft.width(), 58, WONTHOUND_HOTPINK);
        drawGlitchStatus(120, "NO SD", WONTHOUND_HOTPINK);
        drawCenteredText(160, "Insert SD card and retry", WONTHOUND_GUNMETAL, 1);
        return;
    }
            ...
            case PHASE_HUB:
                ...
                if (phase == PHASE_WD_LIST) {
                    ...
                    if (tvLineCount == 0) {
                        ...
                        drawGlitchText(55, tvTitle ? tvTitle : "VIEW", &Nosifer_Regular10pt7b);
                        tft.drawLine(0, 58, tft.width(), 58, WONTHOUND_HOTPINK);
                        drawGlitchStatus(120, "NO DATA", WONTHOUND_HOTPINK);
                        drawCenteredText(160, "File is empty or missing", WONTHOUND_GUNMETAL, 1);
                        delay(1500);
                        freeTextViewer();
                        phase = PHASE_HUB;
                        scanHubInfo();
