...
//
// WontHound Edition — ported by Wontfallo (based on ESP32-DIV)
...
#define GRID_BOTTOM_PAD 30                                // extra scroll room below last row so it never clips

static TFT_eSprite *g_homeSpr = nullptr;
static TFT_eSprite *g_iconSpr = nullptr;   // reusable icon sprite for transparent compositing
...
static void clampMenuScroll() {
    ...
    int contentH = rows * GRID_ROW_H + GRID_BOTTOM_PAD;   // pad so the last row scrolls clear of the edge
...
// Compose the scrolling home grid off-screen (PSRAM sprite) and push it once for
// flicker-free drag scrolling. Falls back to direct draw if the sprite can't allocate.
void drawMenuFrame() {
    clampMenuScroll();
    if (!g_homeSpr) {
        g_homeSpr = new TFT_eSprite(&tft);
        g_homeSpr->setColorDepth(16);
        if (!g_homeSpr->createSprite(SCREEN_WIDTH, SCREEN_HEIGHT)) {
            delete g_homeSpr;
            g_homeSpr = nullptr;   // fall back to direct draw
        }
    }

    // Everything (bg + transparent icons + labels) composes into ONE sprite and is
    // pushed in a single pass — no flicker. Icons use a reusable icon-sprite + pushToSprite
    // because TFT_eSprite::pushImage has no transparent-color overload.
    if (g_homeSpr) {
        TFT_eSprite &s = *g_homeSpr;
        if (!g_iconSpr) {
            g_iconSpr = new TFT_eSprite(&tft);
            g_iconSpr->setColorDepth(16);
            if (!g_iconSpr->createSprite(WH_ICON_W, WH_ICON_H)) { delete g_iconSpr; g_iconSpr = nullptr; }
        }
        s.setSwapBytes(true);
        s.pushImage(0, 0, WONTHOUND_BG_W, WONTHOUND_BG_H, wonthound_bg);
        s.setViewport(0, GRID_TOP, SCREEN_WIDTH, GRID_BOTTOM - GRID_TOP);   // clip content to window
        ...
        for (int i = 0; i < NUM_MENU_ITEMS; i++) {
            int x, y;
            menuTileRect(i, &x, &y);
            if (y > GRID_BOTTOM || y + GRID_TILE < GRID_TOP) continue;       // fully off-screen
            if (g_iconSpr) {
                // Load icon WITHOUT swap: pushToSprite writes through the destination
                // sprite's swap (which is ON), giving exactly one swap to match the bg.
                g_iconSpr->setSwapBytes(false);
                g_iconSpr->pushImage(0, 0, WH_ICON_W, WH_ICON_H, (uint16_t *)wh_menu_icons[i]);
                g_iconSpr->pushToSprite(&s, x, y, WH_ICON_TRANSPARENT);
            }
            s.drawString(menu_items[i], (i % GRID_COLS) * GRID_CELL_W + GRID_CELL_W / 2, y + WH_ICON_H + 2);
        }
        s.resetViewport();
        s.setTextDatum(TL_DATUM);
        s.pushSprite(0, 0);
    } else {
        // Fallback (no PSRAM sprite): draw directly; icons via transparent-color pushImage.
        tft.setSwapBytes(true);
        tft.pushImage(0, 0, WONTHOUND_BG_W, WONTHOUND_BG_H, wonthound_bg);
        tft.setViewport(0, GRID_TOP, SCREEN_WIDTH, GRID_BOTTOM - GRID_TOP);
        ...
        for (int i = 0; i < NUM_MENU_ITEMS; i++) {
            int x, y;
            menuTileRect(i, &x, &y);
            if (y > GRID_BOTTOM || y + GRID_TILE < GRID_TOP) continue;
            tft.pushImage(x, y, WH_ICON_W, WH_ICON_H, (uint16_t *)wh_menu_icons[i], WH_ICON_TRANSPARENT);
            tft.setTextColor(TFT_WHITE);
            tft.drawString(menu_items[i], (i % GRID_COLS) * GRID_CELL_W + GRID_CELL_W / 2, y + WH_ICON_H + 2);
        }
        tft.resetViewport();
        tft.setTextDatum(TL_DATUM);
...
void handleButtons() {
    ...
    if (in_sub_menu) {
        ...
        if (peekTouchPoint(&tpx, &tpy)) {
            ...
            int startScroll = menu_scroll_y;
            bool dragged = false;
            last_interaction_time = millis();

            // Follow the finger while it stays down.
            while (isStillTouched()) {
                uint16_t cx, cy;
                if (peekTouchPoint(&cx, &cy)) {
                    int dy = (int)cy - startY;
                    if (!dragged && abs(dy) > 6) dragged = true;   // tap vs drag threshold
                    if (dragged) {
                        menu_scroll_y = startScroll - dy;
                        clampMenuScroll();
                        drawMenuFrame();
                    }
                }
            ...
            if (dragged) return;   // it was a scroll, not a tap

            // ---- Tap handling (use the finger-down position) ----
            // Lock icon (top-right)
            ...
            // Tile tap — hit-test the WHOLE cell (icon + label + padding), not just the
            // small icon, so taps anywhere on the tile register (much more responsive).
            if (startY >= GRID_TOP && startY <= GRID_BOTTOM) {
                for (int i = 0; i < NUM_MENU_ITEMS; i++) {
                    int x, y;
                    menuTileRect(i, &x, &y);
                    int cellX = (i % GRID_COLS) * GRID_CELL_W;
                    if (startX >= cellX && startX <= cellX + GRID_CELL_W &&
                        startY >= y && startY <= y + GRID_ROW_H) {
                        current_menu_index = i;
                        tft.drawRect(x, y, GRID_TILE, GRID_TILE, WONTHOUND_HOTPINK);   // tap flash
                        delay(60);
                        updateActiveSubmenu();
