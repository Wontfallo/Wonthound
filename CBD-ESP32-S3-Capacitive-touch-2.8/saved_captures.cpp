// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Saved Captures Browser
...
static void dumpToSerial(int idx) {
    ...
    Serial.println();
    Serial.println("════════════════════════════════════════════");
    Serial.printf("  WONTHOUND CAPTURE DUMP: %s\n", fe.name);
    Serial.println("════════════════════════════════════════════");
...
static void drawListHeader() {
    ...
    // Separator line
    tft.drawFastHLine(4, SC_LIST_Y - 2, 232, WONTHOUND_VIOLET);
}
...
static void drawFileRow(int screenRow, int fileIdx, bool selected) {
    ...
    // Clear row
    tft.fillRect(2, y, SCREEN_WIDTH - 4, SC_ROW_HEIGHT - 1, selected ? WONTHOUND_VIOLET : TFT_BLACK);
    ...
    // Type badge
    uint16_t badgeColor = (f.type == FT_HC22000) ? WONTHOUND_HOTPINK : WONTHOUND_VIOLET;
...
static void drawEmptyScreen() {
    ...
    drawCenteredText(160, "No captures found in /eapol/", WONTHOUND_GUNMETAL, 1);
    drawCenteredText(180, "Run EAPOL Capture first", WONTHOUND_GUNMETAL, 1);

    // SD card status
    if (!sdMounted) {
        drawCenteredText(210, "SD CARD NOT DETECTED", WONTHOUND_HOTPINK, 1);
    }
}
...
static void drawDetailView() {
    ...
    // Frame box
    tft.drawRoundRect(10, 64, CONTENT_INNER_W, 100, 4, WONTHOUND_VIOLET);
    tft.drawRoundRect(11, 65, CONTENT_INNER_W - 2, 98, 4, WONTHOUND_VIOLET);
    ...
    // VIEW button
    tft.drawRoundRect(scBtn1X, 180, scBtnW, 32, 4, WONTHOUND_MAGENTA);
    tft.drawRoundRect(scBtn1X + 1, 181, scBtnW - 2, 30, 4, WONTHOUND_MAGENTA);
    tft.setTextDatum(TC_DATUM);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.drawString("VIEW", scBtn1X + scBtnW / 2, 190);

    // SERIAL button
    tft.drawRoundRect(scBtn2X, 180, scBtnW, 32, 4, WONTHOUND_VIOLET);
    tft.drawRoundRect(scBtn2X + 1, 181, scBtnW - 2, 30, 4, WONTHOUND_VIOLET);
    tft.setTextColor(WONTHOUND_VIOLET);
    tft.drawString("SERIAL", scBtn2X + scBtnW / 2, 190);

    // DELETE button
    tft.drawRoundRect(scBtn3X, 180, scBtnW, 32, 4, WONTHOUND_HOTPINK);
    tft.drawRoundRect(scBtn3X + 1, 181, scBtnW - 2, 30, 4, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawDeleteConfirm() {
    ...
    tft.drawRoundRect(21, 121, 198, 78, 4, WONTHOUND_HOTPINK);
    ...
    // YES / NO buttons
    tft.drawRoundRect(35, 168, 70, 24, 3, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.drawString("YES", 70, 173);

    tft.drawRoundRect(135, 168, 70, 24, 3, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void drawViewScreen() {
    ...
    // Frame box
    tft.drawRoundRect(5, 82, GRAPH_PADDED_W, viewLineCount * 16 + 16, 4, WONTHOUND_VIOLET);
    ...
    for (int i = 0; i < viewLineCount && i < SC_VIEW_LINES; i++) {
        ...
        if (strncmp(viewLines[i], "TYPE:", 5) == 0) {
            ...
            tft.setTextColor(WONTHOUND_MAGENTA);  // Hex data
...
static void handleDetailTouch(int tx, int ty) {
    ...
    if (tx >= 85 && tx <= 155 && ty >= 180 && ty <= 212) {
        ...
        tft.fillRoundRect(86, 181, 68, 30, 4, WONTHOUND_VIOLET);
        tft.setTextDatum(TC_DATUM);
        ...
        // Show confirmation
        tft.fillRoundRect(86, 181, 68, 30, 4, WONTHOUND_MAGENTA);
        tft.setTextDatum(TC_DATUM);
        tft.setTextColor(TFT_BLACK);
        ...
        tft.drawRoundRect(85, 180, 70, 32, 4, WONTHOUND_VIOLET);
        tft.drawRoundRect(86, 181, 68, 30, 4, WONTHOUND_VIOLET);
        tft.setTextDatum(TC_DATUM);
        tft.setTextColor(WONTHOUND_VIOLET);
...
static void handleConfirmTouch(int tx, int ty) {
    ...
    if (tx >= 35 && tx <= 105 && ty >= 168 && ty <= 192) {
        if (deleteFile(selectedIndex)) {
            ...
            tft.drawRoundRect(20, 120, 200, 80, 4, WONTHOUND_HOTPINK);
            tft.setTextDatum(TC_DATUM);
            tft.setTextColor(WONTHOUND_HOTPINK);
