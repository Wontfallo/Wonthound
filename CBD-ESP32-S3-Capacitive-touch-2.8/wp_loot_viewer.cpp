// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD WhisperPair Loot Viewer
...
static void dumpToSerial(int idx) {
    ...
    Serial.println();
    Serial.println("════════════════════════════════════════════");
    Serial.printf("  WONTHOUND WP LOOT DUMP: %s\n", fe.name);
    Serial.println("════════════════════════════════════════════");
...
static void drawListHeader() {
    ...
    // Separator line
    tft.drawFastHLine(4, WL_LIST_Y - 2, 232, WONTHOUND_VIOLET);
}
...
static void drawFileRow(int screenRow, int fileIdx, bool selected) {
    ...
    // Clear row
    tft.fillRect(2, y, SCREEN_WIDTH - 4, WL_ROW_HEIGHT - 1, selected ? WONTHOUND_VIOLET : TFT_BLACK);
...
static void drawEmptyScreen() {
    ...
    drawCenteredText(160, "No loot found in /wp_loot/", WONTHOUND_GUNMETAL, 1);
    drawCenteredText(180, "Run WhisperPair attack first", WONTHOUND_GUNMETAL, 1);
    ...
    if (!sdMounted) {
        ...
        drawCenteredText(240, heapBuf, WONTHOUND_GUNMETAL, 1);
    }
}
...
static void drawDetailView() {
    ...
    // VIEW button
    tft.drawRoundRect(btn1X, 180, btnW, 32, 4, WONTHOUND_MAGENTA);
    tft.drawRoundRect(btn1X + 1, 181, btnW - 2, 30, 4, WONTHOUND_MAGENTA);
    tft.setTextDatum(TC_DATUM);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.drawString("VIEW", btn1X + btnW / 2, 190);

    // SERIAL button
    tft.drawRoundRect(btn2X, 180, btnW, 32, 4, WONTHOUND_VIOLET);
    tft.drawRoundRect(btn2X + 1, 181, btnW - 2, 30, 4, WONTHOUND_VIOLET);
    tft.setTextColor(WONTHOUND_VIOLET);
    tft.drawString("SERIAL", btn2X + btnW / 2, 190);

    // DELETE button
    tft.drawRoundRect(btn3X, 180, btnW, 32, 4, WONTHOUND_HOTPINK);
    tft.drawRoundRect(btn3X + 1, 181, btnW - 2, 30, 4, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawViewPage1() {
    ...
    // Separator
    tft.drawFastHLine(6, y, SCREEN_WIDTH - 12, WONTHOUND_DARK);
    y += 6;
    ...
    for (int i = 0; i < pLoot->strategyCount; i++) {
        ...
        if (strstr(pLoot->strategies[i], "HIT")) {
            ...
            tft.setTextColor(WONTHOUND_GUNMETAL);  // Gray for N/A
        }
        tft.setCursor(14, y);
...
static void handleDetailTouch(int tx, int ty) {
    ...
    if (tx >= btn2X && tx <= btn2X + btnW && ty >= 180 && ty <= 212) {
        ...
        tft.fillRoundRect(btn2X + 1, 181, btnW - 2, 30, 4, WONTHOUND_VIOLET);
        tft.setTextDatum(TC_DATUM);
        ...
        // Show confirmation
        tft.fillRoundRect(btn2X + 1, 181, btnW - 2, 30, 4, WONTHOUND_MAGENTA);
        tft.setTextDatum(TC_DATUM);
        tft.setTextColor(TFT_BLACK);
