// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD RFID/NFC Attack Module Implementation
...
// ═══════════════════════════════════════════════════════════════════════════
// UI HELPERS — Matching standard WontHound visual style
// ═══════════════════════════════════════════════════════════════════════════

// WontHound signature gradient: Cyan (0,207,255) → Hot Pink (255,28,82)
...
static void drawGradientBar(int x, int y, int w, int h, int fillW) {
    ...
    tft.drawRoundRect(x - 2, y - 2, w + 4, h + 4, 3, WONTHOUND_VIOLET);
    tft.drawRoundRect(x - 1, y - 1, w + 2, h + 2, 2, WONTHOUND_GUNMETAL);
...
static void drawCardFlash() {
    for (int f = 0; f < 2; f++) {
        uint16_t col = (f == 0) ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
...
static void drawScrollBar(int offset, int total, int visible) {
    ...
    if (total <= visible) {
        drawCenteredRFID(SCROLL_BAR_Y + 3, "BACK to exit", WONTHOUND_GUNMETAL);
        return;
    }
    ...
    drawCenteredRFID(SCROLL_BAR_Y + 3, buf, WONTHOUND_GUNMETAL);
...
void setup() {
    ...
    tft.setTextSize(TEXT_SIZE_BODY);
    drawCenteredRFID(SCAN_FW_Y, "Initializing PN532...", WONTHOUND_MAGENTA);
    ...
    if (!pn532Init()) {
        ...
        int y = SCAN_FW_Y;
        drawCenteredRFID(y, "PN532 NOT FOUND!", WONTHOUND_HOTPINK);
        y += 18;
        ...
        y += 22;
        drawCenteredRFID(y, "Tap BACK to exit", WONTHOUND_GUNMETAL);
        return;
    }
    ...
    drawCenteredRFID(SCAN_STATUS_Y, "Hold card to reader...", WONTHOUND_MAGENTA);
...
void loop() {
    ...
    if (success) {
        ...
        if (isNew) {
            ...
            tft.setTextColor(WONTHOUND_GREEN, TFT_BLACK);
            tft.setCursor(10, SCAN_BOTTOM_Y);
...
static void drawReaderPage() {
    ...
    for (int i = 0; i < maxVisible && (displayOffset + i) < totalSectors; i++) {
        ...
        tft.setTextColor(sectorReadOk[s] ? rfidGradientColor((float)s / totalSectors) : WONTHOUND_HOTPINK, TFT_BLACK);
        tft.setCursor(2, ry);
        tft.printf("S%02d:", s);
        tft.setTextColor(WONTHOUND_GUNMETAL, TFT_BLACK);
        tft.setCursor(28, ry);
...
void setup() {
    ...
    if (!pn532Init()) {
        drawCenteredRFID(RDR_INFO_Y, "PN532 NOT FOUND!", WONTHOUND_HOTPINK);
        drawCenteredRFID(RDR_INFO_Y + 18, "Check wiring & retry", WONTHOUND_GUNMETAL);
        drawCenteredRFID(RDR_INFO_Y + 36, "Tap BACK to exit", WONTHOUND_GUNMETAL);
        return;
    }

    drawCenteredRFID(RDR_INFO_Y + 6, "Hold MIFARE card to reader", WONTHOUND_MAGENTA);
    drawCenteredRFID(RDR_INFO_Y + 22, "to read all sectors", WONTHOUND_GUNMETAL);
}
        ...
        case READER_WAIT_CARD: {
            ...
            if (detected) {
                ...
                if (!isMifareClassic(cardSak)) {
                    ...
                    drawRFIDTitle(RDR_TITLE_Y, "CARD READER");
                    drawRFIDSeparator(RDR_SEP_Y);
                    drawCenteredRFID(RDR_INFO_Y, "Not a MIFARE Classic!", WONTHOUND_HOTPINK);
                    tft.setTextColor(WONTHOUND_MAGENTA, TFT_BLACK);
                    tft.setCursor(10, RDR_INFO_Y + 18);
                    tft.printf("SAK: 0x%02X  %s", cardSak, getCardTypeName(cardSak));
                    drawCenteredRFID(RDR_INFO_Y + 38, "Only Classic 1K/4K supported", WONTHOUND_GUNMETAL);
                    state = READER_DONE;
                    break;
                }
...
void setup() {
    ...
    if (!pn532Init()) {
        drawCenteredRFID(CLN_INFO_Y, "PN532 NOT FOUND!", WONTHOUND_HOTPINK);
        drawCenteredRFID(CLN_INFO_Y + 18, "Tap BACK to exit", WONTHOUND_GUNMETAL);
        return;
    }
    ...
    drawCenteredRFID(CLN_INFO_Y + 18, "Hold SOURCE card to reader", WONTHOUND_MAGENTA);
    drawCenteredRFID(CLN_INFO_Y + 34, "(card to copy FROM)", WONTHOUND_GUNMETAL);
}
...
void loop() {
    ...
    switch (state) {
        case CLONE_WAIT_SOURCE: {
            ...
            if (detected) {
                ...
                if (!isMifareClassic(sourceSak)) {
                    ...
                    drawRFIDTitle(CLN_TITLE_Y, "CARD CLONE");
                    drawRFIDSeparator(CLN_SEP_Y);
                    drawCenteredRFID(CLN_INFO_Y, "Not MIFARE Classic!", WONTHOUND_HOTPINK);
                    drawCenteredRFID(CLN_INFO_Y + 18, "Tap BACK to exit", WONTHOUND_GUNMETAL);
                    state = CLONE_DONE;
                    break;
                }
        ...
        case CLONE_READ_SOURCE: {
            ...
            drawCenteredRFID(CLN_MSG_Y + 40, "Remove source card", WONTHOUND_MAGENTA);
            drawCenteredRFID(CLN_MSG_Y + 56, "then hold BLANK card", WONTHOUND_MAGENTA);
            drawCenteredRFID(CLN_MSG_Y + 74, "(Magic Gen1a for UID clone)", WONTHOUND_GUNMETAL);
        ...
        case CLONE_WAIT_BLANK: {
            ...
            if (blankDetected) {
                ...
                int y = CLN_BAR_Y + 22;
                if (ok) {
                    drawCenteredRFID(y, "CLONE SUCCESSFUL!", WONTHOUND_GREEN);
                } else {
                    drawCenteredRFID(y, "Clone had errors", WONTHOUND_HOTPINK);
                    y += 16;
                    drawCenteredRFID(y, "Some sectors may not match", WONTHOUND_GUNMETAL);
                }

                y += 22;
                drawRFIDSeparator(y);
                y += 8;
                drawCenteredRFID(y, "NOTE: UID unchanged unless", WONTHOUND_GUNMETAL);
                y += 14;
                drawCenteredRFID(y, "using Magic Gen1a card", WONTHOUND_GUNMETAL);
...
void setup() {
    ...
    if (!pn532Init()) {
        drawCenteredRFID(BRT_INFO_Y, "PN532 NOT FOUND!", WONTHOUND_HOTPINK);
        drawCenteredRFID(BRT_INFO_Y + 18, "Tap BACK to exit", WONTHOUND_GUNMETAL);
        return;
    }

    drawCenteredRFID(BRT_INFO_Y + 6, "Hold MIFARE card to reader", WONTHOUND_MAGENTA);
    drawCenteredRFID(BRT_INFO_Y + 22, "52 keys x 16 sectors", WONTHOUND_GUNMETAL);
}
...
void loop() {
    ...
    if (!bruteRunning && !bruteDone) {
        ...
        if (detected) {
            ...
            if (!isMifareClassic(sak)) {
                ...
                drawRFIDTitle(BRT_TITLE_Y, "KEY BRUTE");
                drawRFIDSeparator(BRT_SEP_Y);
                drawCenteredRFID(BRT_INFO_Y, "Not MIFARE Classic!", WONTHOUND_HOTPINK);
                bruteDone = true;
                return;
            }
    ...
    if (bruteDone && !resultsShown) {
        ...
        drawCenteredRFID(BRT_RESULTS_Y, "BRUTE FORCE COMPLETE", WONTHOUND_GREEN);
    ...
    if (bruteRunning) {
        ...
        tft.setTextColor(foundCount > 0 ? rfidPulseColor(millis()) : WONTHOUND_GUNMETAL, TFT_BLACK);
...
static void drawUidDisplay() {
    ...
    tft.drawRoundRect(8, y - 4, SCREEN_WIDTH - 16, 22, 3, WONTHOUND_VIOLET);
    tft.drawRoundRect(9, y - 3, SCREEN_WIDTH - 18, 20, 2, WONTHOUND_GUNMETAL);
...
static void drawArrowButton(int x, int y, int w, int h, const char* label, uint16_t color) {
    tft.fillRoundRect(x, y, w, h, 4, WONTHOUND_DARK);
...
static void drawEmulateButtons() {
    ...
    drawArrowButton(startX,                          row1Y, EMU_ARROW_W, EMU_ARROW_H, "< L", WONTHOUND_VIOLET);
    drawArrowButton(startX + EMU_ARROW_W + gap,      row1Y, EMU_ARROW_W, EMU_ARROW_H, "UP",  WONTHOUND_CYAN);
    drawArrowButton(startX + (EMU_ARROW_W + gap) * 2, row1Y, EMU_ARROW_W, EMU_ARROW_H, "DN",  WONTHOUND_CYAN);
    drawArrowButton(startX + (EMU_ARROW_W + gap) * 3, row1Y, EMU_ARROW_W, EMU_ARROW_H, "R >", WONTHOUND_VIOLET);

    // Row 2: +16 / -16 (fast scroll full byte)
    int row2Y = row1Y + EMU_ARROW_H + 6;
    int halfW = (SCREEN_WIDTH - 30) / 2;
    drawArrowButton(10,            row2Y, halfW, EMU_ARROW_H, "+16", WONTHOUND_MAGENTA);
    drawArrowButton(10 + halfW + 10, row2Y, halfW, EMU_ARROW_H, "-16", WONTHOUND_MAGENTA);

    // EMULATE / STOP button
    int emuX = (SCREEN_WIDTH - EMU_EMU_W) / 2;
    if (emulating) {
        tft.fillRoundRect(emuX, EMU_EMU_BTN_Y, EMU_EMU_W, EMU_EMU_H, 5, WONTHOUND_HOTPINK);
        ...
        tft.fillRoundRect(emuX, EMU_EMU_BTN_Y, EMU_EMU_W, EMU_EMU_H, 5, WONTHOUND_GREEN);
        tft.setTextColor(TFT_BLACK);
        tft.setTextSize(TEXT_SIZE_BODY);
...
static void drawEmulateStatus() {
    ...
    if (emulating) {
        drawCenteredRFID(EMU_STATUS_Y, "EMULATING...", WONTHOUND_GREEN);
        drawCenteredRFID(EMU_STATUS_Y + 16, "Waiting for reader", WONTHOUND_MAGENTA);
        ...
        drawCenteredRFID(EMU_STATUS_Y, "Ready to emulate", WONTHOUND_GUNMETAL);
    }
}
...
void setup() {
    ...
    if (!pn532Init()) {
        drawCenteredRFID(EMU_UID_Y, "PN532 NOT FOUND!", WONTHOUND_HOTPINK);
        drawCenteredRFID(EMU_UID_Y + 18, "Tap BACK to exit", WONTHOUND_GUNMETAL);
        return;
    }
...
namespace {
bool rfidStubExit = false;

void showRFIDUnavailable(const char* title) {
    (void)title;
    rfidStubExit = false;
    // PN532 is intentionally disabled for builds without an NFC wiring target.
    rfidStubExit = true;
}
...
namespace RFIDScanner {
void setup() { showRFIDUnavailable("RFID SCANNER"); }
void loop() { rfidStubLoop(); }
void cleanup() { rfidStubExit = false; }
bool isExitRequested() { return rfidStubExit; }
}

namespace RFIDReader {
void setup() { showRFIDUnavailable("RFID READER"); }
void loop() { rfidStubLoop(); }
void cleanup() { rfidStubExit = false; }
bool isExitRequested() { return rfidStubExit; }
}
...
namespace RFIDClone {
void setup() { showRFIDUnavailable("RFID CLONE"); }
void loop() { rfidStubLoop(); }
void cleanup() { rfidStubExit = false; }
bool isExitRequested() { return rfidStubExit; }
}
...
namespace RFIDBrute {
void setup() { showRFIDUnavailable("RFID BRUTE"); }
void loop() { rfidStubLoop(); }
void cleanup() { rfidStubExit = false; }
bool isExitRequested() { return rfidStubExit; }
}
...
namespace RFIDEmulate {
void setup() { showRFIDUnavailable("RFID EMULATE"); }
void loop() { rfidStubLoop(); }
void cleanup() { rfidStubExit = false; }
bool isExitRequested() { return rfidStubExit; }
}
