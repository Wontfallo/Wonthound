// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD WiFi Attack Modules Implementation
...
// Initialize color palette - WONTHOUND CYBERPUNK (black → purple → blue → pink → white)
...
static void drawFftFrame() {
    ...
    tft.fillRect(center_x, area_graph_y, center_x, area_graph_height, WONTHOUND_BLACK);
    for (int j = 0; j < (int)half_width; j++) {
        int k = pmKValues[j];
        unsigned int color = palette_red[k] << 11 | palette_green[k] << 5 | palette_blue[k];
    ...
    tft.fillRect(0, area_graph_y, center_x, area_graph_height, WONTHOUND_BLACK);
    for (int j = 0; j < (int)half_width; j++) {
        int k = pmKValues[j];
        unsigned int color = palette_red[k] << 11 | palette_green[k] << 5 | palette_blue[k];
        int current_y = area_graph_height - map(k, 0, 127, 0, area_graph_height) + area_graph_y;
...
static void doSamplingFFT() {
    ...
    tft.fillRect(center_x, area_graph_y, half_width, area_graph_height, WONTHOUND_BLACK);
    ...
    tft.fillRect(0, area_graph_y, half_width, area_graph_height, WONTHOUND_BLACK);
...
#define PM_ICON_NUM 3
    ...
    tft.drawLine(0, pm_sep_y, SCREEN_WIDTH, pm_sep_y, WONTHOUND_MAGENTA);
...
void loop() {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // ICON BAR TOUCH HANDLING - Must run before touchButtonsUpdate().
    // The S3 FT6336 build maps the top-left icon bar to BTN_BACK, so the
    // generic virtual-button layer can consume the same touch first.
    ...
    // Update generic virtual buttons after module-specific icon handling.
    touchButtonsUpdate();

    if (isBackButtonTapped() || buttonPressed(BTN_BACK)) {
        exitRequested = true;
        return;
    }
...
static const char* ssidList[] = {
    ...
    "Meme_LANd", "WontHound_Was_Here", "Fenrir_Network", "DRAUGR_AP"
};
...
static void updateRainDrop(int idx) {
    ...
    for (int i = 0; i < TRAIL_LENGTH; i++) {
        ...
        if (charY >= RAIN_AREA_TOP && charY < RAIN_AREA_BOTTOM && drop->trail[i] != 0) {
            ...
            tft.fillRect(drop->x, charY, 6, CHAR_HEIGHT, WONTHOUND_BLACK);
    ...
    if (eraseY >= RAIN_AREA_TOP && eraseY < RAIN_AREA_BOTTOM) {
        tft.fillRect(drop->x, eraseY, 6, CHAR_HEIGHT, WONTHOUND_BLACK);
    }
...
void nukeMode() {
    ...
    while (true) {
        ...
        if (millis() - lastFlash > 200) {
            ...
            tft.setTextSize(2);
            tft.setTextColor(flashState ? WONTHOUND_HOTPINK : 0x041F);
            tft.setCursor(SCALE_X(30), SCALE_Y(25));
            tft.print("!! NUKE MODE !!");
            ...
            tft.setTextSize(1);
            tft.setTextColor(WONTHOUND_BRIGHT);
            tft.fillRect(0, SCALE_Y(50), SCREEN_WIDTH, 10, WONTHOUND_BLACK);
...
static void initAttackMode() {
    ...
    strncpy((char*)ap_config.ap.ssid, "WontHound", sizeof(ap_config.ap.ssid));
...
// Draw button - WONTHOUND THEME (cyan text on dark bg)
static void drawButton(int x, int y, int w, int h, const char* label, bool highlight, bool disabled) {
    uint16_t bgColor = disabled ? WONTHOUND_GUNMETAL : WONTHOUND_DARK;
    uint16_t borderColor = highlight ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
    uint16_t textColor = highlight ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
...
static void drawTabBar(const char* leftButton, bool leftDisabled, const char* prevButton, bool prevDisabled, const char* nextButton, bool nextDisabled) {
    ...
    tft.fillRect(0, tabY, SCREEN_WIDTH, 16, WONTHOUND_GUNMETAL);
...
static void drawScanScreen() {
    ...
    for (int i = startIdx; i < endIdx && y < listEndY; i++) {
        ...
        if (i == highlightedIndex && selectedApIndex == -1) {
            tft.fillRect(0, y - 1, SCREEN_WIDTH, rowHeight, WONTHOUND_DARK);
            tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawSkullSpinner() {
    ...
    if (attackRunning) {
        ...
        for (int i = 0; i < numSkulls; i++) {
            ...
            tft.fillRect(x, skullY, 16, 16, WONTHOUND_BLACK);
        ...
        for (int i = 0; i < numSkulls; i++) {
            int x = startX + (i * spacing);
            tft.fillRect(x, skullY, 16, 16, WONTHOUND_BLACK);
            tft.drawBitmap(x, skullY, skullIcons[i], 16, 16, WONTHOUND_GUNMETAL);
        }
    }
}
...
static void drawSpinnerLabel() {
    ...
    tft.fillRect(10, lblY, CONTENT_INNER_W, 15, WONTHOUND_BLACK);
    tft.setCursor(SCALE_X(85), lblY + 3);
    tft.setTextColor(attackRunning ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL);
    tft.setTextSize(1);
...
static void updateAttackStats() {
    ...
    tft.fillRect(10, statsY, SCALE_W(200), SCALE_H(50), WONTHOUND_BLACK);
...
static void drawAttackScreen() {
    ...
    tft.fillRect(0, ICON_BAR_BOTTOM + 1, SCREEN_WIDTH, SCREEN_HEIGHT, WONTHOUND_BLACK);
    tft.setTextSize(1);
    ...
    // Packet burst selector: [-] count [+] - WONTHOUND THEME
    ...
    // Draw [-] button
    tft.fillRect(SCALE_X(70), burstY, SCALE_W(30), 20, WONTHOUND_GUNMETAL);
    tft.drawRect(SCALE_X(70), burstY, SCALE_W(30), 20, WONTHOUND_MAGENTA);
    tft.setCursor(SCALE_X(82), burstY + 5);
    tft.setTextColor(WONTHOUND_BRIGHT);
    tft.print("-");

    // Draw count
    tft.fillRect(SCALE_X(105), burstY, SCALE_W(40), 20, WONTHOUND_BLACK);
    ...
    // Draw [+] button
    tft.fillRect(SCALE_X(150), burstY, SCALE_W(30), 20, WONTHOUND_GUNMETAL);
    tft.drawRect(SCALE_X(150), burstY, SCALE_W(30), 20, WONTHOUND_MAGENTA);
...
static void drawAfTitle() {
    tft.drawLine(0, CONTENT_Y_START, SCREEN_WIDTH, CONTENT_Y_START, WONTHOUND_HOTPINK);
    drawGlitchTitle(SCALE_Y(58), "AUTHFLOOD");
    tft.drawLine(0, SCALE_Y(62), SCREEN_WIDTH, SCALE_Y(62), WONTHOUND_HOTPINK);
}

// Scan screen - network list
static void drawScanScreen() {
    const int contentTop = SCALE_Y(63);
    tft.fillRect(0, contentTop, SCREEN_WIDTH, SCREEN_HEIGHT - contentTop, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < afPerPage && startIdx + i < afNetworkCount; i++) {
        ...
        if (idx == afHighlightIdx) {
            tft.fillRect(0, y - 2, SCREEN_WIDTH, rowH, WONTHOUND_DARK);
            tft.setTextColor(WONTHOUND_HOTPINK);
        } else {
            tft.setTextColor(WONTHOUND_MAGENTA);
        }
...
// Draw START/STOP button — WontHound themed, touchable
static void drawActionButton(bool running) {
    ...
    uint16_t borderColor = running ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
    tft.fillRoundRect(btnX, btnY, btnW, btnH, 5, WONTHOUND_DARK);
...
static void drawAfEqualizer() {
    ...
    tft.fillRect(AF_GRAPH_X, AF_GRAPH_Y, AF_GRAPH_WIDTH, AF_GRAPH_HEIGHT, WONTHOUND_BLACK);

    // Frame border
    tft.drawRect(AF_GRAPH_X - 1, AF_GRAPH_Y - 1, AF_GRAPH_WIDTH + 2, AF_GRAPH_HEIGHT + 2, WONTHOUND_MAGENTA);
...
static void drawAttackScreen() {
    ...
    // Separator
    tft.drawLine(5, SCALE_Y(95), SCREEN_WIDTH - 5, SCALE_Y(95), WONTHOUND_DARK);

    // START/STOP button
    drawActionButton(attackRunning);

    // Separator below button
    tft.drawLine(5, SCALE_Y(140), SCREEN_WIDTH - 5, SCALE_Y(140), WONTHOUND_DARK);
    ...
    if (!attackRunning) {
        ...
        // Footer
        tft.drawLine(5, SCREEN_HEIGHT - 30, SCREEN_WIDTH - 5, SCREEN_HEIGHT - 30, WONTHOUND_DARK);
        tft.setTextColor(WONTHOUND_GUNMETAL);
...
static void updateAttackDisplay() {
    ...
    const int statsH = SCALE_H(18);
    const int col2X = SCALE_X(140);
    tft.fillRect(0, statsY - 2, SCREEN_WIDTH, SCALE_H(65), WONTHOUND_BLACK);
...
void loop() {
                ...
                if (now - lastStateChange > STATE_CHANGE_COOLDOWN) {
                    if (!attackRunning) {
                        ...
                        if (!afInitAttackMode(targetAp.primary)) {
                            tft.fillRect(10, SCALE_Y(270), CONTENT_INNER_W, 12, WONTHOUND_BLACK);
                            tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    if (!targetSelected) {
        ...
        if (buttonPressed(BTN_SELECT) && (now - lastStateChange > STATE_CHANGE_COOLDOWN)) {
            ...
            if (!afInitAttackMode(targetAp.primary)) {
                tft.fillRect(10, 270, 220, 12, WONTHOUND_BLACK);
                tft.setTextColor(WONTHOUND_HOTPINK);
            ...
            tft.fillRect(0, SCALE_Y(140), SCREEN_WIDTH, SCREEN_HEIGHT - SCALE_Y(140), WONTHOUND_BLACK);
            drawActionButton(true);
            tft.drawLine(5, SCALE_Y(140), SCREEN_WIDTH - 5, SCALE_Y(140), WONTHOUND_DARK);
...
// ═══════════════════════════════════════════════════════════════════════════
//
// WontHound v2.7.0 UPGRADE - February 2026
...
static void showEvilTwinPopup(int index) {
    ...
    tft.fillRect(popupX, popupY, popupW, popupH, WONTHOUND_BLACK);
    tft.drawRect(popupX, popupY, popupW, popupH, WONTHOUND_HOTPINK);
    tft.drawRect(popupX + 1, popupY + 1, popupW - 2, popupH - 2, WONTHOUND_MAGENTA);
    ...
    // YES button (left)
    tft.fillRoundRect(popupX + 15, btnY, btnW, btnH, 5, WONTHOUND_DARK);
    ...
    // NO button (right)
    tft.fillRoundRect(popupX + popupW - 85, btnY, btnW, btnH, 5, WONTHOUND_DARK);
...
static void drawStats() {
    ...
    tft.fillRect(0, statsY, SCREEN_WIDTH, 12, WONTHOUND_BLACK);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void drawProbeLog() {
    // Clear log area
    tft.fillRect(0, LIST_START_Y, SCREEN_WIDTH, SCREEN_HEIGHT - LIST_START_Y, WONTHOUND_BLACK);
...
namespace WifiScan {
// ═══════════════════════════════════════════════════════════════════════════
// WIFI SCANNER v2.0 - WontHound Edition
...
static void drawSignalBars(int x, int y, int rssi) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i < bars) {
            ...
            tft.fillRect(barX, barY, 3, barH, WONTHOUND_GUNMETAL);
        }
        barX += 4;
    }
}
...
static void showAttackPopup(int index) {
    ...
    // Background with double border (WontHound style)
    tft.fillRect(popupX, popupY, popupW, popupH, WONTHOUND_BLACK);
    tft.drawRect(popupX, popupY, popupW, popupH, WONTHOUND_HOTPINK);
    tft.drawRect(popupX + 2, popupY + 2, popupW - 4, popupH - 4, WONTHOUND_MAGENTA);

    // Skull icon at top center
    tft.drawBitmap(popupX + (popupW/2) - 8, popupY + SCALE_H(8), bitmap_icon_skull_tools, 16, 16, WONTHOUND_HOTPINK);
    ...
    tft.fillRoundRect(btnStartX, btnY, btnW, btnH, 4, WONTHOUND_DARK);
    ...
    tft.fillRoundRect(cloneX, btnY, btnW, btnH, 4, WONTHOUND_DARK);
    tft.drawRoundRect(cloneX, btnY, btnW, btnH, 4, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    tft.fillRoundRect(cancelX, btnY, btnW, btnH, 4, WONTHOUND_DARK);
    tft.drawRoundRect(cancelX, btnY, btnW, btnH, 4, WONTHOUND_GUNMETAL);
    tft.setTextColor(WONTHOUND_GUNMETAL);
...
static void drawList() {
    ...
    for (int i = 0; i < MAX_VISIBLE_ITEMS && i + listStartIndex < filteredCount; i++) {
        ...
        if (dispIdx == currentIndex) {
            tft.fillRect(0, y - 1, SCREEN_WIDTH, LIST_ITEM_HEIGHT, WONTHOUND_DARK);
            tft.setTextColor(WONTHOUND_HOTPINK);
        } else {
            tft.setTextColor(WONTHOUND_MAGENTA);
        }
        ...
        if (enc == WIFI_AUTH_OPEN) {
            ...
            tft.setTextColor(dispIdx == currentIndex ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA);
            tft.print("WP");
        }

        // SSID (truncated)
        tft.setCursor(22, y + 2);
        tft.setTextColor(dispIdx == currentIndex ? WONTHOUND_HOTPINK : WONTHOUND_BRIGHT);
        ...
        tft.setTextColor(dispIdx == currentIndex ? WONTHOUND_HOTPINK : WONTHOUND_VIOLET);
        if (ch < 10) tft.print(" ");
        tft.print(ch);
    ...
    if (filteredCount > MAX_VISIBLE_ITEMS) {
        ...
        if (listStartIndex > 0) {
            tft.fillRoundRect(10, btnY, btnW, btnH, 3, WONTHOUND_DARK);
            tft.drawRoundRect(10, btnY, btnW, btnH, 3, WONTHOUND_MAGENTA);
            tft.setTextColor(WONTHOUND_MAGENTA);
        ...
        if (listStartIndex + MAX_VISIBLE_ITEMS < filteredCount) {
            tft.fillRoundRect(SCREEN_WIDTH - btnW - 10, btnY, btnW, btnH, 3, WONTHOUND_DARK);
            tft.drawRoundRect(SCREEN_WIDTH - btnW - 10, btnY, btnW, btnH, 3, WONTHOUND_HOTPINK);
            tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawDetails() {
    tft.fillRect(0, STATUS_BAR_HEIGHT + 16, SCREEN_WIDTH, SCREEN_HEIGHT - STATUS_BAR_HEIGHT - 16, WONTHOUND_BLACK);
    drawWifiScanUI();
    drawHeader();
...
static void handleRoot() {
    terminalPrint("[+] Client connected", WONTHOUND_MAGENTA);
    totalClients++;
...
static void handleCapture() {
    ...
    if (psk.length() > 0) {
        terminalPrint("[!!!] PSK CAPTURED: " + psk, WONTHOUND_HOTPINK);
    ...
    if (stage == "email") {
        ...
        terminalPrint("[!] EMAIL: " + email, WONTHOUND_HOTPINK);
        ...
        terminalPrint("[!] PASS: " + password, WONTHOUND_HOTPINK);
        ...
        terminalPrint("[!] MFA: " + mfaCode, WONTHOUND_HOTPINK);
        ...
        if (room.length() > 0) {
            terminalPrint("[!] ROOM: " + room, WONTHOUND_HOTPINK);
            extra = room.c_str();
        }
...
void startPortal() {
    ...
    terminalPrint("[*] EVIL TWIN ACTIVE", WONTHOUND_MAGENTA);
    terminalPrint("[*] SSID: " + String(customSSID), WONTHOUND_MAGENTA);
    terminalPrint("[*] IP: " + WiFi.softAPIP().toString(), WONTHOUND_MAGENTA);
    terminalPrint("[*] CH: " + String(ch), WONTHOUND_MAGENTA);
    terminalPrint("[*] Template: " + String(portalTemplateNames[currentTemplate]), WONTHOUND_HOTPINK);
    if (hasTarget) {
        terminalPrint("[*] CORE 0 DEAUTH ACTIVE", WONTHOUND_MAGENTA);
        terminalPrint("[*] Hammering real AP continuously...", WONTHOUND_MAGENTA);
    } else {
        terminalPrint("[!] Real AP not found — no deauth", WONTHOUND_GUNMETAL);
    }
...
void stopPortal() {
    ...
    terminalPrint("[*] Portal STOPPED", WONTHOUND_HOTPINK);
    drawMainScreen();
...
static void drawHeader() {
    tft.fillRect(0, CONTENT_Y_START, SCREEN_WIDTH, SCALE_H(30), WONTHOUND_BLACK);
    ...
    if (isMultiStage(currentTemplate)) {
        tft.fillRoundRect(badgeX, badgeY, 44, 9, 2, WONTHOUND_VIOLET);
        tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    tft.fillRoundRect(badgeX, badgeY, badgeW, 9, 2, WONTHOUND_DARK);
    tft.drawRoundRect(badgeX, badgeY, badgeW, 9, 2, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawInfo() {
    ...
    tft.fillRect(0, infoY, SCREEN_WIDTH, SCALE_H(21), WONTHOUND_BLACK);

    tft.setTextSize(1);

    // SSID in subtle rounded frame
    tft.drawRoundRect(3, infoY + 1, SCREEN_WIDTH - 6, SCALE_H(12), 2, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_GUNMETAL);
    ...
    if (portalActive) {
        ...
        tft.drawCircle(9, statusY + 3, 3, WONTHOUND_GUNMETAL);
        tft.setTextColor(WONTHOUND_GUNMETAL);
        tft.setCursor(16, statusY);
        tft.print("IDLE");
    }
...
static void drawTerminal() {
    tft.fillRect(0, TERM_Y_START, SCREEN_WIDTH, TERM_Y_END - TERM_Y_START, WONTHOUND_BLACK);

    // Thin border around terminal area
    tft.drawRect(2, TERM_Y_START, SCREEN_WIDTH - 4, TERM_Y_END - TERM_Y_START, WONTHOUND_DARK);
...
static void drawStats() {
    ...
    tft.drawLine(5, statsY, SCREEN_WIDTH - 5, statsY, WONTHOUND_DARK);
    ...
    // Separator
    tft.setTextColor(WONTHOUND_DARK);
    ...
    tft.print("Creds:");
    tft.setTextColor(credCount > 0 ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL);
...
static void drawButtonBar() {
    ...
    tft.fillRect(0, barTop, SCREEN_WIDTH, SCREEN_HEIGHT - barTop, WONTHOUND_BLACK);
    tft.drawLine(0, barTop + 1, SCREEN_WIDTH, barTop + 1, WONTHOUND_DARK);

    // BACK button — outlined hotpink
    tft.drawRoundRect(5, btnY, btnW, btnH, 4, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setTextSize(1);
    ...
    if (portalActive) {
        tft.fillRoundRect(btn2X, btnY, btnW, btnH, 4, WONTHOUND_HOTPINK);
        tft.setTextColor(WONTHOUND_BLACK);
        tft.setCursor(btn2X + SCALE_W(20), btnY + SCALE_H(8));
        tft.print("STOP");
    } else {
        tft.fillRoundRect(btn2X, btnY, btnW, btnH, 4, WONTHOUND_DARK);
    ...
    tft.drawRoundRect(btn3X, btnY, btnW, btnH, 4, WONTHOUND_VIOLET);
    tft.setTextColor(WONTHOUND_VIOLET);
...
static void drawInputField() {
    ...
    tft.fillRect(12, fieldY, SCREEN_WIDTH - 26, SCALE_H(20), WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setTextSize(2);
...
static void drawInputBox() {
    int boxY = SCALE_Y(55);
    tft.fillRect(10, boxY, SCREEN_WIDTH - 20, SCALE_H(25), WONTHOUND_DARK);
    tft.drawRect(10, boxY, SCREEN_WIDTH - 20, SCALE_H(25), WONTHOUND_HOTPINK);
    drawInputField();
}
...
static void drawKeyboard() {
    ...
    for (int row = 0; row < 4; row++) {
        ...
        for (int col = 0; col < (int)strlen(keyboardLayout[row]); col++) {
            tft.fillRect(xOffset, yOffset, keyWidth, keyHeight, WONTHOUND_DARK);
            tft.drawRect(xOffset, yOffset, keyWidth, keyHeight, WONTHOUND_GUNMETAL);
            tft.setTextColor(WONTHOUND_MAGENTA);
            tft.setTextSize(1);
    ...
    // Back button
    tft.fillRoundRect(5, btnY, kbBtnW, kbBtnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(5, btnY, kbBtnW, kbBtnH, 3, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    tft.fillRoundRect(autoX, btnY, kbBtnW, kbBtnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(autoX, btnY, kbBtnW, kbBtnH, 3, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    // OK button
    int okX = SCALE_X(165);
    tft.fillRoundRect(okX, btnY, kbBtnW, kbBtnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(okX, btnY, kbBtnW, kbBtnH, 3, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void handleKeyboard(int x, int y) {
    ...
    for (int row = 0; row < 4; row++) {
        ...
        for (int col = 0; col < (int)strlen(keyboardLayout[row]); col++) {
            if (x >= xOffset && x <= xOffset + keyWidth &&
                ...
                // Visual feedback
                tft.fillRect(xOffset, yOffset, keyWidth, keyHeight, WONTHOUND_HOTPINK);
                tft.setTextColor(WONTHOUND_BLACK);
    ...
    if (x >= autoX && x <= (autoX + kbBtnW) && y >= btnY && y <= btnY + kbBtnH + 3) {
        if (inputSSID.length() > 0) {
            ...
            tft.fillRect(5, SCALE_Y(210), SCREEN_WIDTH - 10, SCALE_H(12), WONTHOUND_BLACK);
            tft.setTextColor(WONTHOUND_HOTPINK);
...
static void drawCredList() {
    ...
    // BACK button
    tft.fillRoundRect(5, btnY, credBtnW, credBtnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(5, btnY, credBtnW, credBtnH, 3, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    if (count > 8) {
        tft.fillRoundRect(pageX, btnY, credBtnW, credBtnH, 3, WONTHOUND_DARK);
        tft.drawRoundRect(pageX, btnY, credBtnW, credBtnH, 3, WONTHOUND_MAGENTA);
        tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    tft.fillRoundRect(clearX, btnY, credBtnW, credBtnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(clearX, btnY, credBtnW, credBtnH, 3, WONTHOUND_VIOLET);
    tft.setTextColor(WONTHOUND_VIOLET);
...
void setup() {
    ...
    for (int i = 0; i < TERM_MAX_LINES; i++) {
        terminalBuffer[i] = "";
        colorBuffer[i] = WONTHOUND_MAGENTA;
    }
    ...
    terminalPrint("[*] Evil Twin ready", WONTHOUND_MAGENTA);
    terminalPrint("[*] Template: " + String(portalTemplateNames[currentTemplate]), WONTHOUND_VIOLET);
...
void loop() {
    ...
    if (portalActive) {
        ...
        if (cpDeauthRunning && (millis() - lastDeauthReport >= 5000)) {
            ...
            terminalPrint("[*] DEAUTH: " + String(cnt) + " (" + String(rate) + "%)", WONTHOUND_MAGENTA);
        ...
        if (currentClients > prevClientCount) {
            ...
            for (int i = 0; i < newClients; i++) {
                terminalPrint("[+] New device connected", WONTHOUND_MAGENTA);
            }
            totalClients += newClients;
            drawInfo();
            drawStats();
        } else if (currentClients < prevClientCount) {
            terminalPrint("[-] Device disconnected", WONTHOUND_GUNMETAL);
            drawInfo();
        }
...
void clearAllCredentials() {
    EEPROM.write(COUNT_ADDR, 0);
    EEPROM.commit();
    terminalPrint("[*] All credentials cleared", WONTHOUND_HOTPINK);
}
...
// WontHound station discovery implementation
// ═══════════════════════════════════════════════════════════════════════════
...
static void drawHeaders() {
    int hdrY = SCALE_Y(60);
    tft.fillRect(0, hdrY, SCREEN_WIDTH, SCALE_H(14), WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setTextSize(1);
...
static void drawStationList() {
    ...
    for (int i = listStartIndex; i < endIdx && y < listEnd; i++) {
        ...
        if (s->selected) {
            ...
            rowColor = WONTHOUND_BRIGHT;  // Has AP - most useful!
        } else if (age < 5000) {
            rowColor = WONTHOUND_MAGENTA;    // Recently active probe
        } else if (age > 30000) {
        ...
        if (i == currentIndex) {
            tft.fillRect(0, y, SCREEN_WIDTH, ITEM_HEIGHT - 2, WONTHOUND_DARK);
        }
...
static void showStationInfo(int idx) {
    ...
    tft.fillRect(10, popY, popW, popH, WONTHOUND_DARK);
    tft.drawRect(10, popY, popW, popH, WONTHOUND_HOTPINK);
    ...
    tft.fillRect(closeBtnX, closeBtnY, SCALE_W(70), SCALE_H(20), WONTHOUND_DARK);
    tft.drawRect(closeBtnX, closeBtnY, SCALE_W(70), SCALE_H(20), WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void drawButtonBar() {
    ...
    tft.fillRect(0, selStatusY, SCREEN_WIDTH, SCALE_H(10), WONTHOUND_BLACK);
    tft.setTextSize(1);
    if (selCount > 0) {
    ...
    tft.fillRect(0, barY, SCREEN_WIDTH, SCALE_H(37), WONTHOUND_GUNMETAL);
    ...
    tft.fillRect(5, btnRowY, backW, btnH, WONTHOUND_DARK);
    tft.drawRect(5, btnRowY, backW, btnH, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    uint16_t infoColor = (currentIndex >= 0 && currentIndex < stationCount) ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    tft.fillRect(infoX, btnRowY, infoW, btnH, WONTHOUND_DARK);
    ...
    int navW = SCALE_W(25);
    uint16_t prevColor = canPrev ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    tft.fillRect(prevX, btnRowY, navW, btnH, WONTHOUND_DARK);
    ...
    tft.fillRect(pageX, btnRowY, pageW, btnH, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    uint16_t nextColor = canNext ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    tft.fillRect(nextX, btnRowY, navW, btnH, WONTHOUND_DARK);
    ...
    int atkW = SCALE_W(43);
    uint16_t atkColor = (selCount > 0 && selAssociated > 0) ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL;
    tft.fillRect(atkX, btnRowY, atkW, btnH, WONTHOUND_DARK);
