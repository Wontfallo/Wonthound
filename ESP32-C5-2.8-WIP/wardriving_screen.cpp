// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Wardriving Screen
...
static void drawStartStopButton(bool active) {
    // Clear button area
    tft.fillRect(WD_BTN_X - 2, WD_BTN_Y - 2, WD_BTN_W + 4, WD_BTN_H + 4, WONTHOUND_BLACK);

    if (active) {
        // STOP button — hotpink border, hotpink text
        tft.drawRoundRect(WD_BTN_X, WD_BTN_Y, WD_BTN_W, WD_BTN_H, 8, WONTHOUND_HOTPINK);
        tft.drawRoundRect(WD_BTN_X + 1, WD_BTN_Y + 1, WD_BTN_W - 2, WD_BTN_H - 2, 7, WONTHOUND_HOTPINK);
        ...
        tft.drawRoundRect(WD_BTN_X, WD_BTN_Y, WD_BTN_W, WD_BTN_H, 8, WONTHOUND_MAGENTA);
        tft.drawRoundRect(WD_BTN_X + 1, WD_BTN_Y + 1, WD_BTN_W - 2, WD_BTN_H - 2, 7, WONTHOUND_MAGENTA);
        tft.setFreeFont(&Nosifer_Regular10pt7b);
...
static void drawWDScreen() {
    ...
    // Main stats frame
    tft.drawRoundRect(WD_FRAME_X, WD_FRAME_Y, WD_FRAME_W, WD_FRAME_H, 6, WONTHOUND_VIOLET);
    tft.drawRoundRect(WD_FRAME_X + 1, WD_FRAME_Y + 1, WD_FRAME_W - 2, WD_FRAME_H - 2, 5, WONTHOUND_GUNMETAL);
    ...
    // Separator
    tft.drawLine(WD_FRAME_X, WD_STATS_Y + SCALE_H(33), WD_FRAME_X + WD_FRAME_W, WD_STATS_Y + SCALE_H(33), WONTHOUND_HOTPINK);
    ...
    // Separator
    tft.drawLine(WD_FRAME_X, WD_GPS_Y + SCALE_H(24), WD_FRAME_X + WD_FRAME_W, WD_GPS_Y + SCALE_H(24), WONTHOUND_HOTPINK);
    ...
    // Separator
    tft.drawLine(WD_FRAME_X, WD_SPEED_Y + SCALE_H(12), WD_FRAME_X + WD_FRAME_W, WD_SPEED_Y + SCALE_H(12), WONTHOUND_HOTPINK);
...
static void updateWDValues() {
    ...
    tft.fillRect(WD_FRAME_X + 3, WD_FRAME_Y + 3, WD_FRAME_W - 6, WD_FRAME_H - 6, WONTHOUND_BLACK);
    ...
    // NETWORKS value
    tft.fillRect(WD_COL_L_VAL1, WD_STATS_Y, SCALE_W(55), 8, WONTHOUND_BLACK);
    tft.setTextColor(stats.active ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
    tft.setCursor(WD_COL_L_VAL1, WD_STATS_Y);
    tft.print(stats.newNetworks);

    // OPEN value
    tft.fillRect(WD_COL_R_VAL, WD_STATS_Y, SCALE_W(75), 8, WONTHOUND_BLACK);
    tft.setTextColor(stats.active ? (stats.openNetworks > 0 ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA) : WONTHOUND_GUNMETAL);
    ...
    // BLE value
    tft.fillRect(WD_COL_L_VAL2, WD_STATS_Y + SCALE_H(10), SCALE_W(85), 8, WONTHOUND_BLACK);
    tft.setTextColor(stats.newBleDevices > 0 ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
    tft.setCursor(WD_COL_L_VAL2, WD_STATS_Y + SCALE_H(10));
    tft.print(stats.newBleDevices);

    // DUPES value (WiFi + BLE combined)
    tft.fillRect(SCALE_X(165), WD_STATS_Y + SCALE_H(10), SCALE_W(65), 8, WONTHOUND_BLACK);
    tft.setTextColor(stats.active ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
    ...
    // SCANS value
    tft.fillRect(WD_COL_L_VAL3, WD_STATS_Y + SCALE_H(20), SCALE_W(65), 8, WONTHOUND_BLACK);
    ...
    tft.fillRect(SCALE_X(170), WD_STATS_Y + SCALE_H(20), SCALE_W(65), 8, WONTHOUND_BLACK);
    ...
    if (stats.active) {
        ...
        if (wdBlinkState) {
            ...
            tft.fillCircle(SCALE_X(205), WD_STATS_Y + SCALE_H(24), 3, WONTHOUND_HOTPINK);
        } else {
            tft.setTextColor(WONTHOUND_GUNMETAL);
            tft.print(isBle ? "BLE" : "WIFI");
            tft.fillCircle(SCALE_X(205), WD_STATS_Y + SCALE_H(24), 3, WONTHOUND_GUNMETAL);
        }
    } else {
        tft.setTextColor(WONTHOUND_GUNMETAL);
        tft.print("IDLE");
    ...
    tft.fillRect(WD_COL_L_VAL2, WD_GPS_Y, SCALE_W(85), 8, WONTHOUND_BLACK);
    ...
    // SATS value
    tft.fillRect(WD_COL_R_VAL, WD_GPS_Y, SCALE_W(50), 8, WONTHOUND_BLACK);
    tft.setTextColor(gpsData.satellites > 0 ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
    ...
    tft.fillRect(WD_COL_L_VAL2, WD_GPS_Y + SCALE_H(12), SCALE_W(90), 8, WONTHOUND_BLACK);
    ...
    tft.fillRect(SCALE_X(150), WD_GPS_Y + SCALE_H(12), SCALE_W(85), 8, WONTHOUND_BLACK);
    ...
    tft.fillRect(WD_COL_L_VAL4, WD_SPEED_Y, SCALE_W(70), 8, WONTHOUND_BLACK);
    ...
    tft.fillRect(WD_COL_R_VAL, WD_SPEED_Y, SCALE_W(80), 8, WONTHOUND_BLACK);
    ...
    // ── SD File ──
    tft.fillRect(10, WD_FILE_Y + SCALE_H(14), SCREEN_WIDTH - 20, 8, WONTHOUND_BLACK);
    tft.setTextSize(1);
...
static void wdRunBleScan() {
    ...
    if (!foundDevices) {
        Serial.println("[WARDRIVING] BLE getResults() returned NULL — skipping BLE results");
        wdBleScan->stop();
        wdBleScan->clearResults();
        wdBleScan = nullptr;
        BLEDevice::deinit(false);
