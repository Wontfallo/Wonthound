// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD EAPOL/PMKID Capture Module
...
// WontHound extracts the PMKID on-device and writes hashcat-ready .hc22000.
// ═══════════════════════════════════════════════════════════════════════════
...
// Deauth spinner — skull loading animation with WontHound gradient
...
static void drawAPList() {
    // Clear list area
    tft.fillRect(0, SCALE_Y(60), SCREEN_WIDTH, SCREEN_HEIGHT - SCALE_Y(60), WONTHOUND_BLACK);
    ...
    tft.print("ENC");
    tft.drawLine(5, SCALE_Y(72), GRAPH_PADDED_W, SCALE_Y(72), WONTHOUND_VIOLET);
    ...
    for (int i = startIdx; i < endIdx; i++) {
        ...
        tft.setTextColor(hasEAPOL ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
        tft.setCursor(10, y);
...
static void drawCaptureScreen() {
    ...
    tft.drawLine(0, SCALE_Y(58), SCREEN_WIDTH, SCALE_Y(58), WONTHOUND_MAGENTA);

    // Target info frame
    tft.drawRoundRect(5, SCALE_Y(62), GRAPH_PADDED_W, SCALE_H(30), 6, WONTHOUND_MAGENTA);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    tft.drawLine(5, SCALE_Y(198), GRAPH_PADDED_W, SCALE_Y(198), WONTHOUND_MAGENTA);
    ...
    tft.drawLine(5, EC_DEAUTH_Y - 10, GRAPH_PADDED_W, EC_DEAUTH_Y - 10, WONTHOUND_MAGENTA);

    // Deauth button — skull icon + text
    tft.drawRoundRect(EC_DEAUTH_X, EC_DEAUTH_Y, EC_DEAUTH_W, EC_DEAUTH_H, 6, WONTHOUND_HOTPINK);
    tft.drawRoundRect(EC_DEAUTH_X + 1, EC_DEAUTH_Y + 1, EC_DEAUTH_W - 2, EC_DEAUTH_H - 2, 5, WONTHOUND_HOTPINK);
    tft.drawBitmap(EC_DEAUTH_X + 8, EC_DEAUTH_Y + 8, bitmap_icon_skull_wifi, 16, 16, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(EC_DEAUTH_X + 30, EC_DEAUTH_Y + 12);
    tft.print("DEAUTH");

    // Save button (dimmed until we have something) — SD card icon + text
    tft.drawRoundRect(EC_SAVE_X, EC_SAVE_Y, EC_SAVE_W, EC_SAVE_H, 6, WONTHOUND_GUNMETAL);
    tft.drawBitmap(EC_SAVE_X + 8, EC_SAVE_Y + 8, bitmap_icon_sdcard, 16, 16, WONTHOUND_GUNMETAL);
    tft.setTextColor(WONTHOUND_GUNMETAL);
...
static void updateCaptureDisplay() {
    ...
    int hsY = SCALE_Y(97);
    tft.fillRect(SCALE_X(85), hsY, SCALE_W(150), 16, WONTHOUND_BLACK);

    int mx = SCALE_X(85);
    int mboxW = SCALE_W(30);
    int mboxGap = SCALE_W(35);
    uint16_t m1c = hasMsg1 ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    uint16_t m2c = hasMsg2 ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    uint16_t m3c = hasMsg3 ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    uint16_t m4c = hasMsg4 ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    ...
    if (hasHandshake) {
        tft.fillRect(SCALE_X(85), SCALE_Y(113), SCALE_W(150), 10, WONTHOUND_BLACK);
        tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    // ── PMKID status ──
    tft.fillRect(SCALE_X(55), SCALE_Y(125), SCALE_W(180), 16, WONTHOUND_BLACK);
    ...
    // ── Stats ──
    tft.fillRect(SCALE_X(65), SCALE_Y(153), SCALE_W(170), SCALE_H(40), WONTHOUND_BLACK);
    ...
    tft.fillRect(5, statusAreaY, GRAPH_PADDED_W, statusAreaH, WONTHOUND_BLACK);
    ...
    if (hasPMKID && hasHandshake) {
        ...
        // Animated skull spinner with WontHound gradient
        ...
        for (int d = 0; d < GRADIENT_COUNT; d++) {
            uint16_t dc = (d <= deauthAnimFrame % GRADIENT_COUNT) ? gradientColors[d] : WONTHOUND_GUNMETAL;
    ...
    if ((hasPMKID || hasHandshake) && !savedPMKID && !savedHandshake) {
        tft.drawRoundRect(EC_SAVE_X, EC_SAVE_Y, EC_SAVE_W, EC_SAVE_H, 6, WONTHOUND_MAGENTA);
        tft.drawRoundRect(EC_SAVE_X + 1, EC_SAVE_Y + 1, EC_SAVE_W - 2, EC_SAVE_H - 2, 5, WONTHOUND_MAGENTA);
        tft.drawBitmap(EC_SAVE_X + 8, EC_SAVE_Y + 8, bitmap_icon_sdcard, 16, 16, WONTHOUND_MAGENTA);
        tft.setTextColor(WONTHOUND_MAGENTA);
        tft.setCursor(EC_SAVE_X + 30, EC_SAVE_Y + 12);
        tft.print("SAVE");
    } else if (savedPMKID || savedHandshake) {
        tft.drawRoundRect(EC_SAVE_X, EC_SAVE_Y, EC_SAVE_W, EC_SAVE_H, 6, WONTHOUND_MAGENTA);
        tft.drawBitmap(EC_SAVE_X + 8, EC_SAVE_Y + 8, bitmap_icon_save, 16, 16, WONTHOUND_MAGENTA);
        tft.setTextColor(WONTHOUND_MAGENTA);
        tft.setCursor(EC_SAVE_X + 30, EC_SAVE_Y + 12);
        tft.print("SAVED");
    }
    ...
    if (currentPhase == PHASE_SCAN) {
        ...
        if (isDeauthTapped()) {
            ...
            tft.fillRoundRect(EC_DEAUTH_X + 2, EC_DEAUTH_Y + 2,
                              EC_DEAUTH_W - 4, EC_DEAUTH_H - 4, 4, WONTHOUND_BLACK);
            if (deauthRunning) {
                tft.drawRoundRect(EC_DEAUTH_X, EC_DEAUTH_Y, EC_DEAUTH_W, EC_DEAUTH_H, 6, WONTHOUND_MAGENTA);
                tft.drawRoundRect(EC_DEAUTH_X + 1, EC_DEAUTH_Y + 1, EC_DEAUTH_W - 2, EC_DEAUTH_H - 2, 5, WONTHOUND_MAGENTA);
                tft.fillRoundRect(EC_DEAUTH_X + 2, EC_DEAUTH_Y + 2,
                                  EC_DEAUTH_W - 4, EC_DEAUTH_H - 4, 4, WONTHOUND_MAGENTA);
                tft.drawBitmap(EC_DEAUTH_X + 8, EC_DEAUTH_Y + 8, bitmap_icon_skull_wifi, 16, 16, WONTHOUND_BLACK);
        ...
        if (isSaveTapped() && (hasPMKID || hasHandshake)) {
            ...
            tft.fillRoundRect(EC_SAVE_X + 2, EC_SAVE_Y + 2,
                              EC_SAVE_W - 4, EC_SAVE_H - 4, 4, WONTHOUND_MAGENTA);
            tft.drawBitmap(EC_SAVE_X + 8, EC_SAVE_Y + 8, bitmap_icon_save, 16, 16, WONTHOUND_BLACK);
            tft.setTextColor(WONTHOUND_BLACK);
            tft.setCursor(EC_SAVE_X + 30, EC_SAVE_Y + 12);
            tft.print("SAVED!");
            delay(500);
            ...
            tft.fillRoundRect(EC_SAVE_X + 2, EC_SAVE_Y + 2,
                              EC_SAVE_W - 4, EC_SAVE_H - 4, 4, WONTHOUND_BLACK);
            tft.drawRoundRect(EC_SAVE_X, EC_SAVE_Y, EC_SAVE_W, EC_SAVE_H, 6, WONTHOUND_MAGENTA);
            tft.drawRoundRect(EC_SAVE_X + 1, EC_SAVE_Y + 1, EC_SAVE_W - 2, EC_SAVE_H - 2, 5, WONTHOUND_MAGENTA);
            tft.drawBitmap(EC_SAVE_X + 8, EC_SAVE_Y + 8, bitmap_icon_sdcard, 16, 16, WONTHOUND_MAGENTA);
            tft.setTextColor(WONTHOUND_MAGENTA);
        ...
        if ((hasPMKID || hasHandshake) && !notifiedCapture) {
            ...
            for (int flash = 0; flash < 4; flash++) {
                tft.fillRect(5, alertY, GRAPH_PADDED_W, alertH, (flash % 2) ? WONTHOUND_BLACK : WONTHOUND_MAGENTA);
                tft.setTextSize(2);
                tft.setTextColor((flash % 2) ? WONTHOUND_MAGENTA : WONTHOUND_BLACK);
                if (hasPMKID && hasHandshake) {
