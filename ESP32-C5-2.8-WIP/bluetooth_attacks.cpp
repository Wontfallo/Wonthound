// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Bluetooth Attack Modules Implementation
...
static esp_err_t esp_ble_gap_set_rand_addr(uint8_t* addr) {
    ...
    return BLEDevice::setOwnAddr(addr) ? ESP_OK : ESP_FAIL;
}
...
static esp_err_t esp_ble_gap_config_adv_data_raw(uint8_t* data, uint32_t len) {
    ...
    advData.addData(reinterpret_cast<char*>(data), len);
...
static esp_err_t esp_ble_gap_config_scan_rsp_data_raw(uint8_t* data, uint32_t len) {
    ...
    scanData.addData(reinterpret_cast<char*>(data), len);
...
static void getApplePopupPayload(BLEAdvertisementData& advData, int idx) {
    ...
    advData.addData(reinterpret_cast<char*>(pkt), 31);
}
...
static void getSourApplePayload(BLEAdvertisementData& advData, int typeIdx) {
    ...
    // Random tail (3 bytes)
    esp_fill_random(&pkt[i], 3);
    advData.addData(reinterpret_cast<char*>(pkt), 17);
}
...
static void getFastPairPayload(BLEAdvertisementData& advData, int idx) {
    ...
    pkt[i++] = model[1];
    pkt[i++] = model[2];
    advData.addData(reinterpret_cast<char*>(pkt), 14);
}
...
static void getSamsungWatchPayload(BLEAdvertisementData& advData, int idx) {
    ...
    advData.addData(reinterpret_cast<char*>(pkt), 18);
}
...
static void getSwiftPairPayload(BLEAdvertisementData& advData, int idx) {
    ...
    i += nameLen;
    advData.addData(reinterpret_cast<char*>(pkt), i);
}


// ═══════════════════════════════════════════════════════════════════════════
// UI DRAWING FUNCTIONS — Enhanced WontHound Visual Suite
...
static void drawEqualizer() {
    ...
    // Frame around EQ
    tft.drawRect(SP_EQ_X - 2, SP_EQ_Y - 1, SP_EQ_WIDTH + 4, SP_EQ_HEIGHT + 2, WONTHOUND_GUNMETAL);
    ...
    if (!hasHeat && !spamming) {
        ...
        for (int i = 0; i < SP_EQ_BARS; i++) {
            ...
            tft.fillRect(x + 1, barY, barWidth - 2, barH, WONTHOUND_GUNMETAL);
        }
        return;
    }
    ...
    for (int i = 0; i < SP_EQ_BARS; i++) {
        ...
        if (heat > 80) {
            tft.drawFastHLine(x + 1, barY + barH, barWidth - 2, WONTHOUND_HOTPINK);
        }
    }
}
    ...
    for (int row = 0; row < SP_SKULL_ROWS; row++) {
        ...
        for (int i = 0; i < SP_SKULL_NUM; i++) {
            ...
            if (spamming) {
                ...
                color = WONTHOUND_GUNMETAL;  // Gray when inactive
            }
        ...
        if (row == 0) {
            ...
            tft.setTextColor(spamming ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, TFT_BLACK);
            tft.setTextSize(1);
...
static void drawCounter() {
    ...
    // Border
    tft.drawRoundRect(barX - 1, barY - 1, barW + 2, barH + 2, 2, WONTHOUND_MAGENTA);
    tft.fillRoundRect(barX, barY, barW, barH, 1, WONTHOUND_DARK);
    ...
    if (spamming) {
        tft.drawBitmap(SCALE_X(220), counterY + 1, bitmap_icon_skull_bluetooth, 16, 16, WONTHOUND_HOTPINK);
    }
}
...
static void nextMode() {
    ...
    addLogEntry(buf, WONTHOUND_HOTPINK);
}
...
static void doBroadcast() {
    ...
    String payload = advData.getPayload();
    esp_ble_gap_config_adv_data_raw((uint8_t*)payload.c_str(), payload.length());
...
static void bcnDrawHeader() {
    ...
    // Mode in framed bar
    tft.drawRoundRect(5, SCALE_Y(82), GRAPH_PADDED_W, SCALE_H(16), 3, WONTHOUND_VIOLET);
    tft.drawRoundRect(6, SCALE_Y(83), GRAPH_PADDED_W - 2, SCALE_H(14), 2, WONTHOUND_GUNMETAL);
    tft.setTextColor(WONTHOUND_MAGENTA, TFT_BLACK);
    ...
    tft.drawLine(0, SCALE_Y(100), SCREEN_WIDTH, SCALE_Y(100), WONTHOUND_HOTPINK);

    // Rate display bar
    tft.fillRect(0, SCALE_Y(102), SCREEN_WIDTH, SCALE_H(12), TFT_BLACK);
    tft.setTextColor(beaconing ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, TFT_BLACK);
...
static void doBroadcast() {
    ...
    bcnAddLog(logBuf, WONTHOUND_VIOLET);
}
...
static void startBeacon() {
    beaconing = true;
    rateWindowCount = 0;
    bcnAddLog("[!] BEACON STARTED", WONTHOUND_HOTPINK);
    bcnDrawIconBar();
    bcnDrawHeader();
    bcnDrawLog();
...
static void stopBeacon() {
    ...
    bcnAddLog("[!] BEACON STOPPED", WONTHOUND_HOTPINK);
    bcnDrawIconBar();
    bcnDrawHeader();
...
static void nextMode() {
    ...
    bcnAddLog(buf, WONTHOUND_HOTPINK);
}
...
void setup() {
    ...
    bcnAddLog("[*] BLE Beacon ready", WONTHOUND_MAGENTA);
    char modeBuf[42];
    snprintf(modeBuf, sizeof(modeBuf), "[*] Mode: %s", BCN_MODE_NAMES[currentMode]);
    bcnAddLog(modeBuf, WONTHOUND_MAGENTA);
    bcnDrawLog();
...
static void drawDeviceList() {
    tft.fillRect(0, 37, SCREEN_WIDTH, SCREEN_HEIGHT - 37, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < MAX_VISIBLE_DEVICES && i + listStartIndex < deviceCount; i++) {
        ...
        if (idx == currentIndex) {
            tft.fillRect(0, y - 2, SCREEN_WIDTH, DEVICE_LINE_HEIGHT, WONTHOUND_DARK);
...
void startScan() {
    ...
    if (BLEScanResults* results = pBleScan->start(5, false)) {
        scanResults = *results;
    }
    deviceCount = scanResults.getCount();
    currentIndex = 0;
...
static uint16_t batteryColor(uint8_t level) {
    switch (level) {
        case 0: return WONTHOUND_GREEN;
        case 1: return WONTHOUND_MAGENTA;
        case 2: return WONTHOUND_HOTPINK;
        case 3: return 0xF800;  // Red
        default: return WONTHOUND_GUNMETAL;
    }
}
...
static void drawRssiBar(int x, int y, int rssi) {
    ...
    for (int i = 0; i < 5; i++) {
        int barH = 3 + i * 2;
        int barY = y + (12 - barH);
        uint16_t color = (i < bars) ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL;
...
static void drawTitle() {
    tft.drawLine(0, 38, SCREEN_WIDTH, 38, WONTHOUND_HOTPINK);
    drawGlitchTitle(58, "AIRTAG");
    tft.drawLine(0, 62, SCREEN_WIDTH, 62, WONTHOUND_HOTPINK);
}

static void drawTrackerList() {
    tft.fillRect(0, 63, SCREEN_WIDTH, SCREEN_HEIGHT - 63, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < AT_MAX_VISIBLE && i + listStartIndex < trackerCount; i++) {
        ...
        if (idx == currentIndex) {
            tft.fillRect(0, y - 2, SCREEN_WIDTH, AT_LINE_HEIGHT, WONTHOUND_DARK);
        }
        ...
        uint16_t textCol = (idx == currentIndex) ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
        ...
        // New indicator
        if (t.isNew) {
            tft.fillCircle(SCREEN_WIDTH - 8, y + 5, 3, WONTHOUND_HOTPINK);
        }
...
static void drawTrackerDetail() {
    ...
    tft.fillRect(10, y, barWidth, 10, WONTHOUND_HOTPINK);
    tft.drawRect(10, y, 200, 10, WONTHOUND_VIOLET);
    y += 18;
...
static void drawAlertFlash() {
    ...
    uint16_t color = on ? WONTHOUND_HOTPINK : WONTHOUND_BLACK;
...
static void runScan() {
    ...
    if (BLEScanResults* scanResultsPtr = pAtScan->start(AT_SCAN_DURATION, false)) {
        results = *scanResultsPtr;
    }
...
static uint16_t typeColor(TrackerType t) {
    switch (t) {
        case TRACKER_GOOGLE_FMDN:  return WONTHOUND_GREEN;
        case TRACKER_SAMSUNG_TAG:  return WONTHOUND_VIOLET;
        case TRACKER_TILE:         return WONTHOUND_BRIGHT;
        case TRACKER_CHIPOLO:      return WONTHOUND_MAGENTA;
        case TRACKER_APPLE_AIRTAG: return WONTHOUND_HOTPINK;
        default:                   return WONTHOUND_GUNMETAL;
    }
}
...
static void tdDrawIconBar() {
    tft.drawLine(0, 19, SCREEN_WIDTH, 19, WONTHOUND_MAGENTA);
    tft.fillRect(0, 20, SCREEN_WIDTH, 16, WONTHOUND_GUNMETAL);
    tft.drawBitmap(10, 20, bitmap_icon_go_back, TD_ICON_SIZE, TD_ICON_SIZE, WONTHOUND_MAGENTA);
    tft.drawBitmap(210, 20, bitmap_icon_undo, TD_ICON_SIZE, TD_ICON_SIZE, WONTHOUND_MAGENTA);
    tft.drawLine(0, 36, SCREEN_WIDTH, 36, WONTHOUND_HOTPINK);
}
...
static void tdDrawTitle() {
    ...
    drawGlitchStatus(66, "FRINGE", WONTHOUND_VIOLET);
    tft.drawLine(0, 72, SCREEN_WIDTH, 72, WONTHOUND_HOTPINK);
}

static void drawDeviceList() {
    tft.fillRect(0, 73, SCREEN_WIDTH, SCREEN_HEIGHT - 73, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < TD_MAX_VISIBLE && i + listStartIndex < deviceCount; i++) {
        ...
        if (idx == currentIndex) {
            tft.fillRect(0, y - 2, SCREEN_WIDTH, TD_LINE_HEIGHT, WONTHOUND_DARK);
        }
...
static void drawDeviceDetail() {
    ...
    if (d.type == TRACKER_APPLE_AIRTAG) {
        ...
        uint16_t batColors[] = {WONTHOUND_GREEN, WONTHOUND_MAGENTA, WONTHOUND_HOTPINK, (uint16_t)0xF800};
...
static void tdRunScan() {
    ...
    BLEScanResults results;
    if (BLEScanResults* scanResultsPtr = pTdScan->start(TD_SCAN_DURATION, false)) {
        results = *scanResultsPtr;
    }
    processScanResults(results);
...
static void pfDrawHeader() {
    ...
    if (flooding) {
        ...
        uint16_t statusColor = blink ? WONTHOUND_HOTPINK : tft.color565(200, 50, 100);
        tft.setTextColor(statusColor, TFT_BLACK);
        tft.setCursor(80, 68);
        tft.print(">> FLOODING <<");
    } else {
    ...
    tft.drawRoundRect(5, 74, GRAPH_PADDED_W, 16, 3, WONTHOUND_VIOLET);
    tft.drawRoundRect(6, 75, GRAPH_PADDED_W - 2, 14, 2, WONTHOUND_GUNMETAL);
    tft.setTextColor(WONTHOUND_MAGENTA, TFT_BLACK);
    tft.setCursor(10, 78);
    ...
    tft.drawLine(0, 92, SCREEN_WIDTH, 92, WONTHOUND_HOTPINK);
}
...
static void pfDrawStaticInfo() {
    ...
    // EQ bar frame (static)
    tft.drawRect(11, PF_BAR_Y - 1, PF_BAR_TOTAL_W + 2, 20, WONTHOUND_GUNMETAL);
...
static void pfDrawActivity() {
    ...
    if (flooding) {
        ...
        for (int i = 0; i < PF_NUM_BARS; i++) {
            int x = 12 + (i * barW);
            int barH = 3 + (i % 3);
            tft.fillRect(x + 1, PF_BAR_Y + 18 - barH, barW - 2, barH, WONTHOUND_GUNMETAL);
        }
    }
}
...
static void pfDrawSkulls() {
    ...
    tft.setTextColor(flooding ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, TFT_BLACK);
    tft.setTextSize(1);
...
static void runScan() {
    ...
    BLEScanResults results;
    if (BLEScanResults* scanResultsPtr = S->pArScan->start(AR_SCAN_DURATION, false)) {
        results = *scanResultsPtr;
    }
    processScanResults(results);
...
static void arDrawTagList() {
    int listY = 78;
    tft.fillRect(0, listY, SCREEN_WIDTH, SCREEN_HEIGHT - listY, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < AR_MAX_VISIBLE && i + S->listStartIndex < S->tagCount; i++) {
        ...
        if (idx == S->currentIndex) {
            tft.fillRect(0, y - 1, SCREEN_WIDTH, AR_LINE_HEIGHT, WONTHOUND_DARK);
        }

        // Selection marker
        if (t.selected) {
            tft.setTextColor(WONTHOUND_HOTPINK, (idx == S->currentIndex) ? WONTHOUND_DARK : TFT_BLACK);
            tft.setCursor(2, y + 3);
            tft.print(">");
        }
        ...
        uint16_t textCol = (idx == S->currentIndex) ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
        tft.setTextColor(textCol, (idx == S->currentIndex) ? WONTHOUND_DARK : TFT_BLACK);
        ...
        // Battery
        tft.setTextColor(arBatteryColor(t.battery), (idx == S->currentIndex) ? WONTHOUND_DARK : TFT_BLACK);
        tft.setCursor(93, y + 3);
        tft.print(arBatteryStr(t.battery));

        // RSSI value
        tft.setTextColor(WONTHOUND_VIOLET, (idx == S->currentIndex) ? WONTHOUND_DARK : TFT_BLACK);
        tft.setCursor(130, y + 3);
        tft.printf("%ddB", t.rssi);

        // Proximity
        tft.setTextColor(WONTHOUND_BRIGHT, (idx == S->currentIndex) ? WONTHOUND_DARK : TFT_BLACK);
...
static void arDrawSkulls() {
    ...
    tft.setTextColor(S->replaying ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, TFT_BLACK);
    tft.setTextSize(1);
...
static void arDrawCounterBar() {
    ...
    if (S->replaying) {
        tft.drawBitmap(SCALE_X(220), AR_BAR_Y + 1, bitmap_icon_skull_bluetooth, 16, 16, WONTHOUND_HOTPINK);
    }
}
...
static void startReplay() {
    ...
    arDrawIconBar();
    arDrawStatus(">> REPLAYING <<", WONTHOUND_HOTPINK);
    arDrawStaticReplayInfo();
    arDrawSkulls();
    arDrawCounterBar();
...
static void stopReplay() {
    ...
    tft.fillRect(0, 63, SCREEN_WIDTH, SCREEN_HEIGHT - 63, TFT_BLACK);
    arDrawIconBar();
    arDrawStatus("SCAN MODE", WONTHOUND_MAGENTA);
    arDrawTagList();
...
void setup() {
    ...
    if (S->tagCount > 0) {
        ...
        arDrawStatus("NO TAGS - RESCAN", WONTHOUND_GUNMETAL);
    }
    arDrawTagList();
...
static void drawBjMainUI() {
    ...
    tft.drawLine(0, 56, SCREEN_WIDTH, 56, WONTHOUND_HOTPINK);

    // Rounded frame for main content (Proto Kill style)
    tft.drawRoundRect(10, 60, CONTENT_INNER_W, 70, 8, WONTHOUND_VIOLET);
    tft.drawRoundRect(11, 61, CONTENT_INNER_W - 2, 68, 7, WONTHOUND_GUNMETAL);
    ...
    if (jamming) {
        bjDrawFreeFont(120, "JAMMING", WONTHOUND_HOTPINK, &Nosifer_Regular10pt7b);
    } else {
        bjDrawFreeFont(120, "STANDBY", WONTHOUND_GUNMETAL, &Nosifer_Regular10pt7b);
    }
    ...
    tft.setTextColor(jamming ? WONTHOUND_HOTPINK : WONTHOUND_VIOLET, TFT_BLACK);
    ...
    tft.drawLine(0, 152, SCREEN_WIDTH, 152, WONTHOUND_HOTPINK);
}
...
static void drawJammerDisplay() {
    ...
    if (!jamming) {
        ...
        if (!hasHeat) {
            ...
            tft.drawRect(BJ_GRAPH_X - 1, BJ_GRAPH_Y - 1, BJ_GRAPH_WIDTH + 2, BJ_GRAPH_HEIGHT + 2, WONTHOUND_MAGENTA);

            for (int i = 0; i < BJ_NUM_BARS; i++) {
                int x = BJ_GRAPH_X + (i * BJ_GRAPH_WIDTH / BJ_NUM_BARS);
                int barH = 8 + (i % 5) * 2;
                int barY = BJ_GRAPH_Y + BJ_GRAPH_HEIGHT - barH - 10;
                tft.drawFastVLine(x, barY, barH, WONTHOUND_GUNMETAL);
                tft.drawFastVLine(x + 1, barY, barH, WONTHOUND_GUNMETAL);
            }
    ...
    for (int i = 0; i < BJ_NUM_BARS; i++) {
        ...
        if (heat > 80) {
            tft.drawFastHLine(x, barY + barH, 2, WONTHOUND_HOTPINK);
...
static void drawSkulls() {
    ...
    for (int row = 0; row < BJ_SKULL_ROWS; row++) {
        ...
        if (row == 0) {
            ...
            tft.setTextColor(jamming ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, TFT_BLACK);
            tft.setTextSize(1);
...
void setup() {
    ...
    if (!nrf24Radio.begin()) {
        tft.setTextColor(WONTHOUND_HOTPINK);
        tft.setTextSize(2);
        drawCenteredText(100, "NRF24 NOT FOUND", WONTHOUND_HOTPINK, 2);
        tft.setTextSize(1);
        drawCenteredText(130, "Check wiring:", WONTHOUND_MAGENTA, 1);
        drawCenteredText(145, ("CE=GPIO" + String(NRF24_CE) + " CSN=GPIO" + String(NRF24_CSN)).c_str(), WONTHOUND_MAGENTA, 1);
        initialized = false;
        return;
    }
...
static void drawHeader() {
    tft.fillRect(0, SCALE_Y(38), SCREEN_WIDTH, SCALE_H(20), WONTHOUND_BLACK);
    ...
    tft.setTextColor(WONTHOUND_VIOLET, WONTHOUND_BLACK);
    tft.setCursor(SCALE_X(195), SCALE_Y(42));
    tft.print(filterNames[currentFilter]);
}

static void drawColumnHeaders() {
    tft.fillRect(0, SCALE_Y(58), SCREEN_WIDTH, SCALE_H(14), WONTHOUND_DARK);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void drawDeviceList() {
    ...
    tft.fillRect(0, listY, SCREEN_WIDTH, listH, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < deviceCount && drawn < BSNIFF_MAX_VISIBLE; i++) {
        ...
        if (visibleIdx + listStartIndex == currentIndex) {
            rowColor = WONTHOUND_BRIGHT;           // Highlighted/selected
        } else if (age < 5000 && d->hasName) {
            rowColor = TFT_WHITE;                  // Recent + named
        } else if (age < 5000) {
            rowColor = WONTHOUND_MAGENTA;             // Recent
        } else if (d->companyId != 0xFFFF && lookupCompanyName(d->companyId) != nullptr) {
            rowColor = WONTHOUND_VIOLET;           // Known manufacturer type
        } else if (age > 30000) {
            rowColor = WONTHOUND_GUNMETAL;         // Stale
        } else {
            rowColor = WONTHOUND_MAGENTA;
        }

        // Highlight selected row background
        if (visibleIdx + listStartIndex == currentIndex) {
            tft.fillRect(0, y, SCREEN_WIDTH, BSNIFF_ITEM_HEIGHT - 2, WONTHOUND_DARK);
        }
        ...
        if (d->frameCount > 5 && age < 10000) {
            tft.fillCircle(SCREEN_WIDTH - SCALE_X(5), y + SCALE_H(8), 2, WONTHOUND_HOTPINK);
        }
...
static void drawStatsLine() {
    ...
    tft.fillRect(0, statsY, SCREEN_WIDTH, SCALE_H(10), WONTHOUND_BLACK);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_MAGENTA, WONTHOUND_BLACK);
    ...
    tft.fillRect(SCALE_X(200), statsY + 1, SCALE_W(40), 8, WONTHOUND_BLACK);
    if (scanning) {
        tft.setTextColor(WONTHOUND_MAGENTA, WONTHOUND_BLACK);
        tft.setCursor(SCALE_X(210), statsY + 1);
        tft.print("LIVE");
    } else {
        tft.setTextColor(WONTHOUND_GUNMETAL, WONTHOUND_BLACK);
...
static void drawButtonBar() {
    ...
    tft.fillRect(0, barY, SCREEN_WIDTH, barH, WONTHOUND_GUNMETAL);

    // BACK button
    tft.fillRect(SCALE_X(5), btnY, SCALE_W(37), btnH, WONTHOUND_DARK);
    tft.drawRect(SCALE_X(5), btnY, SCALE_W(37), btnH, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setTextSize(1);
    ...
    // INFO button
    uint16_t infoColor = (deviceCount > 0) ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    tft.fillRect(SCALE_X(47), btnY, SCALE_W(35), btnH, WONTHOUND_DARK);
    ...
    // PREV button
    uint16_t prevColor = (listStartIndex > 0) ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    tft.fillRect(SCALE_X(87), btnY, SCALE_W(30), btnH, WONTHOUND_DARK);
    ...
    tft.fillRect(SCALE_X(120), btnY, SCALE_W(40), btnH, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(SCALE_X(127), txtY);
    tft.printf("%d/%d", curPage, totalPages);

    // NEXT button
    uint16_t nextColor = (listStartIndex + BSNIFF_MAX_VISIBLE < filtered) ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL;
    tft.fillRect(SCALE_X(163), btnY, SCALE_W(30), btnH, WONTHOUND_DARK);
    ...
    // CLR button
    tft.fillRect(SCALE_X(198), btnY, SCALE_W(35), btnH, WONTHOUND_DARK);
    tft.drawRect(SCALE_X(198), btnY, SCALE_W(35), btnH, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void showDeviceDetail(int idx) {
    ...
    tft.fillRect(ovX, ovY, ovW, ovH, WONTHOUND_BLACK);
    tft.drawRect(ovX, ovY, ovW, ovH, WONTHOUND_HOTPINK);
    tft.drawRect(ovX + 1, ovY + 1, ovW - 2, ovH - 2, WONTHOUND_VIOLET);
    ...
    tft.fillRect(closeX, closeY, closeW, closeH, WONTHOUND_DARK);
    tft.drawRect(closeX, closeY, closeW, closeH, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void wpDrawHeader() {
    tft.drawLine(0, 19, tft.width(), 19, WONTHOUND_MAGENTA);
    tft.fillRect(0, 20, SCREEN_WIDTH, 16, WONTHOUND_DARK);
    tft.drawBitmap(wpIconX[0], wpIconY, bitmap_icon_undo, 16, 16, WONTHOUND_MAGENTA);
    tft.drawBitmap(wpIconX[1], wpIconY, bitmap_icon_go_back, 16, 16, WONTHOUND_MAGENTA);
    tft.drawBitmap(112, wpIconY, bitmap_icon_sdcard, 16, 16, WONTHOUND_MAGENTA);
    tft.drawLine(0, 36, SCREEN_WIDTH, 36, WONTHOUND_HOTPINK);
...
static void wpDrawList() {
    tft.fillRect(0, 78, SCREEN_WIDTH, SCREEN_HEIGHT - 78, WONTHOUND_BLACK);

    // Sub-header bar — dark background like spoofer mode bar
    tft.fillRect(0, 78, SCREEN_WIDTH, 16, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setCursor(5, 82);
    tft.print("Fast Pair Targets: ");
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.print(wpCount);
    tft.drawLine(0, 94, SCREEN_WIDTH, 94, WONTHOUND_HOTPINK);
    ...
    for (int i = 0; i < WP_MAX_VISIBLE && i + wpListStart < wpCount; i++) {
        ...
        if (idx == wpCurIdx) {
            tft.fillRect(0, y - 2, SCREEN_WIDTH, WP_LINE_HEIGHT, WONTHOUND_DARK);
            tft.setTextColor(WONTHOUND_HOTPINK);
            tft.setCursor(5, y);
            tft.print("> ");
        } else {
    ...
    tft.drawLine(0, SCREEN_HEIGHT - 18, SCREEN_WIDTH, SCREEN_HEIGHT - 18, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(5, SCREEN_HEIGHT - 12);
...
static void wpDrawProbeStatus(int line, const char* msg, uint16_t col) {
    int y = 135 + line * 15;
    tft.fillRect(0, y, SCREEN_WIDTH, 14, WONTHOUND_BLACK);
...
static void wpDrawResult() {
    ...
    tft.setTextColor(WONTHOUND_HOTPINK); tft.print(d.name);
    y += 15;
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setCursor(10, y); tft.print("MAC:  ");
    tft.setTextColor(WONTHOUND_HOTPINK); tft.print(d.addrStr);
    y += 15;
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setCursor(10, y); tft.print("RSSI: ");
    tft.setTextColor(WONTHOUND_HOTPINK); tft.print(d.rssi); tft.print(" dBm");
    y += 15;
    tft.setTextColor(WONTHOUND_MAGENTA);
    char modelHex[16];
    snprintf(modelHex, sizeof(modelHex), "0x%06X", d.modelId);
    tft.setCursor(10, y); tft.print("Model: ");
    tft.setTextColor(WONTHOUND_HOTPINK); tft.print(modelHex);
    y += 20;

    // Result banner with rounded border
    tft.drawLine(0, y - 4, SCREEN_WIDTH, y - 4, WONTHOUND_DARK);
    uint16_t rc = wpResultColor(d.result);
    tft.fillRoundRect(8, y, SCREEN_WIDTH - 16, 26, 4, rc);
    tft.drawRoundRect(7, y - 1, SCREEN_WIDTH - 14, 28, 4, WONTHOUND_MAGENTA);
    ...
    tft.drawLine(0, SCREEN_HEIGHT - 38, SCREEN_WIDTH, SCREEN_HEIGHT - 38, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(5, SCREEN_HEIGHT - 12);
    tft.print("BACK");
    ...
    if (d.result == WPR_EXPOSED || d.result == WPR_VULNERABLE) {
        ...
        tft.drawRoundRect(139, SCREEN_HEIGHT - 35, 94, 28, 4, WONTHOUND_MAGENTA);
        tft.drawBitmap(146, SCREEN_HEIGHT - 29, bitmap_icon_sword, 16, 16, WONTHOUND_BLACK);
        tft.setTextColor(WONTHOUND_BLACK);
...
static void wpDoScan() {
    ...
    tft.drawLine(10, 98, 200, 98, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    BLEScanResults results;
    if (BLEScanResults* scanResultsPtr = pWpScan->start(WP_SCAN_SECS, false)) {
        results = *scanResultsPtr;
    }
...
static void wpProbeDevice(int idx) {
    ...
    tft.drawLine(5, 94, 180, 94, WONTHOUND_DARK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(10, 100);
    tft.print(d.name);
    tft.setTextColor(WONTHOUND_VIOLET);
    ...
    wpDrawProbeStatus(0, "Connecting...", WONTHOUND_HOTPINK);
    ...
    wpDrawProbeStatus(0, "Connected!", WONTHOUND_MAGENTA);
    #if CYD_DEBUG
    Serial.printf("[WP] Connected to %s\n", d.addrStr);
    #endif

    // Discover Fast Pair service (try 128-bit then 16-bit UUID)
    wpDrawProbeStatus(1, "Finding FP service...", WONTHOUND_HOTPINK);
    ...
    wpDrawProbeStatus(1, "FP Service FOUND!", WONTHOUND_MAGENTA);
    delay(200);

    // Find Key-Based Pairing characteristic
    wpDrawProbeStatus(2, "Finding KBP char...", WONTHOUND_HOTPINK);
    ...
    wpDrawProbeStatus(2, "KBP Char FOUND!", WONTHOUND_HOTPINK);
    ...
    wpDrawProbeStatus(3, "Sending KBP request...", WONTHOUND_HOTPINK);
    ...
    wpDrawProbeStatus(3, "Waiting for response...", WONTHOUND_HOTPINK);
...
static void wpDrawAtkLog() {
    ...
    tft.fillRect(0, startY, SCREEN_WIDTH, WP_ATK_LOG_LINES * lineH, WONTHOUND_BLACK);
...
static void wpDrawAttackReport() {
    ...
    if (atkResultPage == 0) {
        ...
        if (wpAtkResult.hasBrEdr) {
            tft.fillRoundRect(5, y, SCREEN_WIDTH - 10, 28, 3, WONTHOUND_DARK);
        ...
        tft.setTextColor(intelCount > 0 ? 0x07E0 : WONTHOUND_GUNMETAL);
        tft.print(intelStr);
    ...
    // SAVE button (left)
    tft.fillRoundRect(5, SCREEN_HEIGHT - 34, 70, 26, 4, WONTHOUND_DARK);
    tft.drawRoundRect(4, SCREEN_HEIGHT - 35, 72, 28, 4, WONTHOUND_MAGENTA);
    tft.drawBitmap(10, SCREEN_HEIGHT - 29, bitmap_icon_save, 16, 16, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    if (atkResultPage == 0) {
        ...
        tft.print("1/2");
        tft.drawBitmap(SCREEN_WIDTH - 22, SCREEN_HEIGHT - 29, bitmap_icon_RIGHT, 16, 16, WONTHOUND_VIOLET);
    } else {
        tft.drawBitmap(SCREEN_WIDTH - 60, SCREEN_HEIGHT - 29, bitmap_icon_LEFT, 16, 16, WONTHOUND_VIOLET);
...
static void wpSaveLoot() {
    ...
    if (!SD.begin(SD_CS, SPI, 4000000)) {
        ...
        if (!SD.begin(SD_CS, SPI, 4000000)) {
            ...
            #endif
            tft.fillRect(5, SCREEN_HEIGHT - 55, SCREEN_WIDTH - 10, 14, WONTHOUND_BLACK);
            tft.setTextColor(0xF800);
...
static void wpRunAttack() {
    ...
    if (pWpClient && pWpClient->isConnected()) {
        wpAtkAddLog("Reusing probe link!", WONTHOUND_MAGENTA);
        wpDrawAtkLog();
    } else {
        // Connection dropped — full BLE reinit and reconnect
        wpAtkAddLog("Reconnecting...", WONTHOUND_HOTPINK);
        wpDrawAtkLog();
        ...
        for (int attempt = 1; attempt <= 3 && !connected; attempt++) {
            if (attempt > 1) {
                ...
                wpAtkAddLog(retryBuf, WONTHOUND_HOTPINK);
                wpDrawAtkLog();
                delay(500);
            }
        ...
        wpAtkAddLog("Connected!", WONTHOUND_MAGENTA);
        wpDrawAtkLog();
    }

    // Find FP service
    wpAtkAddLog("Finding FP service...", WONTHOUND_HOTPINK);
    wpDrawAtkLog();
    ...
    wpAtkAddLog("FP service found!", WONTHOUND_MAGENTA);
    wpDrawAtkLog();
    ...
    wpAtkAddLog("KBP char ready", WONTHOUND_MAGENTA);
    wpDrawAtkLog();
    ...
    for (int s = 0; s < 4; s++) {
        ...
        wpAtkAddLog(logBuf, WONTHOUND_HOTPINK);
        wpDrawAtkLog();
        ...
        if (wpNotifRx) {
            ...
            wpAtkAddLog("  No response", WONTHOUND_GUNMETAL);
            wpDrawAtkLog();
        }
    ...
    if (pAcctKey) {
        ...
        wpAtkAddLog("Writing Account Key...", WONTHOUND_HOTPINK);
        wpDrawAtkLog();
        ...
        wpAtkAddLog("AcctKey char not found", WONTHOUND_GUNMETAL);
        wpDrawAtkLog();
    }
    ...
    wpAtkAddLog(pPk ? "Passkey char: FOUND" : "Passkey char: absent",
                pPk ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
    wpDrawAtkLog();
    ...
    wpAtkAddLog(pAd ? "AdditionalData: FOUND" : "AdditionalData: absent",
                pAd ? WONTHOUND_MAGENTA : WONTHOUND_GUNMETAL);
    wpDrawAtkLog();
    ...
    if (pMid && pMid->canRead()) {
        ...
        if (mv.length() >= 3) {
            ...
            snprintf(midStr, sizeof(midStr), "Model ID: 0x%02X%02X%02X",
                     wpAtkResult.modelIdBytes[0], wpAtkResult.modelIdBytes[1],
                     wpAtkResult.modelIdBytes[2]);
            wpAtkAddLog(midStr, WONTHOUND_MAGENTA);
        } else {
            wpAtkAddLog("Model ID: short read", WONTHOUND_GUNMETAL);
        }
    } else {
        wpAtkAddLog("Model ID: not readable", WONTHOUND_GUNMETAL);
    }
    wpDrawAtkLog();
    ...
    if (pDis) {
        ...
        if (pFw && pFw->canRead()) {
            ...
            if (fv.length() > 0) {
                ...
                wpAtkAddLog(fwStr, WONTHOUND_MAGENTA);
            }
        } else {
            wpAtkAddLog("FW Rev: not readable", WONTHOUND_GUNMETAL);
        }
    } else {
        wpAtkAddLog("DevInfo svc: absent", WONTHOUND_GUNMETAL);
    }
    wpDrawAtkLog();
...
// Forward declaration — defined in WontHound-CYD.ino (global scope)
...
static void drawReconView() {
    tft.fillRect(0, ICON_BAR_BOTTOM + 1, SCREEN_WIDTH, SCREEN_HEIGHT - ICON_BAR_BOTTOM - 1, WONTHOUND_BLACK);
    ...
    tft.drawLine(0, SCALE_Y(66), SCREEN_WIDTH, SCALE_Y(66), WONTHOUND_HOTPINK);
}

static void drawReconStatus(int line, const char* msg, uint16_t color) {
    int y = SCALE_Y(68) + line * SCALE_H(14);
    tft.fillRect(SCALE_X(5), y, SCREEN_WIDTH - SCALE_X(10), SCALE_H(14), WONTHOUND_BLACK);
    tft.setTextSize(1);
    tft.setTextColor(color, WONTHOUND_BLACK);
...
static void startRecon() {
    ...
    drawReconStatus(0, buf, WONTHOUND_CYAN);
    ...
    BLEAddress addr(String(addrString.c_str()));
    ...
    if (!connected) {
        ...
        drawReconStatus(1, "Retrying (alt addr type)...", WONTHOUND_HOTPINK);
    ...
    if (!connected) {
        ...
        drawReconStatus(2, "Target not connectable", WONTHOUND_GUNMETAL);
        drawReconStatus(3, "Use REPLAY attack instead", WONTHOUND_GUNMETAL);
    ...
    drawReconStatus(1, "Connected! Discovering...", WONTHOUND_MAGENTA);
    ...
    snprintf(summary, sizeof(summary), "Found %d SVCs, %d CHARs",
             S->hpSvcCount, S->hpCharCount);
    if (displayLine < 15) {
        drawReconStatus(displayLine, summary, WONTHOUND_CYAN);
        displayLine++;
    }
    if (displayLine < 15) {
        drawReconStatus(displayLine, "Building honeypot...", WONTHOUND_HOTPINK);
    }
    delay(1000);
...
static void drawHoneypotStatic() {
    ...
    tft.fillRect(0, SCALE_Y(38), SCREEN_WIDTH, SCALE_H(30), WONTHOUND_BLACK);
    drawGlitchText(SCALE_Y(50), "PREDATOR", &Nosifer_Regular10pt7b);
    ...
    tft.drawLine(0, SCALE_Y(68), SCREEN_WIDTH, SCALE_Y(68), WONTHOUND_HOTPINK);
    ...
    if (realIdx >= 0) {
        ...
        tft.print("Clone: ");
        tft.setTextColor(WONTHOUND_CYAN, WONTHOUND_BLACK);
        tft.print(macStr);
    ...
    tft.fillRect(0, bannerY, SCREEN_WIDTH, SCALE_H(18), WONTHOUND_DARK);
    tft.drawLine(0, bannerY, SCREEN_WIDTH, bannerY, WONTHOUND_VIOLET);
    tft.drawLine(0, bannerY + SCALE_H(17), SCREEN_WIDTH, bannerY + SCALE_H(17), WONTHOUND_VIOLET);
    ...
    // STOP — bright, obvious
    tft.fillRect(stopX, btnY, btnW, btnH, WONTHOUND_DARK);
    tft.drawRect(stopX, btnY, btnW, btnH, WONTHOUND_HOTPINK);
    tft.drawRect(stopX + 1, btnY + 1, btnW - 2, btnH - 2, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(stopX + SCALE_X(14), btnY + SCALE_H(8));
    tft.print("STOP");

    // SAVE
    tft.fillRect(saveX, btnY, btnW, btnH, WONTHOUND_DARK);
    tft.drawRect(saveX, btnY, btnW, btnH, WONTHOUND_MAGENTA);
    tft.drawRect(saveX + 1, btnY + 1, btnW - 2, btnH - 2, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
    tft.setCursor(saveX + SCALE_X(14), btnY + SCALE_H(8));
    tft.print("SAVE");

    // CLEAR
    tft.fillRect(clearX, btnY, btnW, btnH, WONTHOUND_DARK);
    tft.drawRect(clearX, btnY, btnW, btnH, WONTHOUND_VIOLET);
    tft.setTextColor(WONTHOUND_VIOLET);
...
static void drawHoneypotLoot() {
    ...
    tft.fillRect(1, bannerY + 1, SCREEN_WIDTH - 2, bannerH - 2, WONTHOUND_DARK);
    ...
    if (S->hpAdvStarted) {
        ...
        uint16_t dotColor = pulse ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
        ...
        tft.setTextColor(WONTHOUND_GUNMETAL, WONTHOUND_DARK);
    ...
    tft.fillRect(0, statusY, SCREEN_WIDTH, statusH, WONTHOUND_BLACK);
    if (S->hpStatusTime > 0 && (now - S->hpStatusTime) < 3000) {
        tft.setTextColor(S->hpStatusColor, WONTHOUND_BLACK);
    ...
    // Clear loot area
    tft.fillRect(0, lootY, SCREEN_WIDTH, lootH, WONTHOUND_BLACK);
...
static void hpSaveLootToSD() {
    if (S->hpLootCount == 0) {
        hpSetStatus("No loot to save!", WONTHOUND_GUNMETAL);
        return;
    }
...
static void handleHoneypotTouch() {
    ...
    if (ty >= btnY - 5 && ty <= btnY + btnH) {
        ...
        if (tx >= zoneW && tx < zoneW * 2) {
            hpSetStatus("Saving to SD...", WONTHOUND_MAGENTA);
            hpSaveLootToSD();
            return;
        }
        // Right zone → CLEAR
        if (tx >= zoneW * 2) {
            ...
            S->hpConnCount = 0;
            memset(S->hpLoot, 0, sizeof(S->hpLoot));
            hpSetStatus("Loot cleared", WONTHOUND_VIOLET);
            return;
        }
    }
...
static void drawDeviceList() {
    ...
    for (int i = 0; i < S->deviceCount && drawn < BP_MAX_VISIBLE; i++) {
        ...
        if (isCurrent) {
            tft.fillRect(0, y, SCREEN_WIDTH, BP_ITEM_HEIGHT - 2, WONTHOUND_DARK);
        }

        // Color coding (tier-aware)
        uint16_t rowColor;
        if (isCurrent) {
            rowColor = WONTHOUND_BRIGHT;
        } else if (d->selected) {
            rowColor = WONTHOUND_HOTPINK;
            ...
            rowColor = WONTHOUND_VIOLET;
        } else if (age > 30000) {
            rowColor = WONTHOUND_GUNMETAL;
        } else {
            rowColor = WONTHOUND_MAGENTA;
        }

        uint16_t bgColor = isCurrent ? WONTHOUND_DARK : TFT_BLACK;
    ...
    tft.fillRect(0, statsY, SCREEN_WIDTH, SCALE_H(14), WONTHOUND_BLACK);
    tft.setTextSize(1);
    ...
    tft.setTextColor(TFT_RED, WONTHOUND_BLACK);
    tft.setCursor(5, statsY + 1);
    ...
    tft.fillRect(0, footY, SCREEN_WIDTH, SCALE_H(25), WONTHOUND_GUNMETAL);
    int filtered = bpCountFiltered();
    ...
    if (totalPages > 1) {
        ...
        tft.setTextColor(curPage > 1 ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, WONTHOUND_GUNMETAL);
        ...
        tft.setTextColor(curPage < totalPages ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL, WONTHOUND_GUNMETAL);
...
static void drawTargetOverlay() {
    ...
    tft.fillRect(replayX, btnY, btnW, btnH, WONTHOUND_DARK);
    ...
    uint16_t hpColor = canAttack ? WONTHOUND_VIOLET : WONTHOUND_GUNMETAL;
    tft.fillRect(hpotX, btnY, btnW + SCALE_W(10), btnH, WONTHOUND_DARK);
    ...
    // BACK button
    tft.fillRect(backX, btnY, btnW, btnH, WONTHOUND_DARK);
    tft.drawRect(backX, btnY, btnW, btnH, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void drawAttackView() {
    ...
    tft.setCursor(5, y);
    tft.print("TYP: ");
    tft.setTextColor(WONTHOUND_BRIGHT, WONTHOUND_BLACK);
...
static void drawCounterBar() {
    ...
    if (S->replaying) {
        tft.drawBitmap(SCALE_X(220), BP_BAR_Y + 1, bitmap_icon_skull_bluetooth, 16, 16, WONTHOUND_HOTPINK);
    }
}
...
static const uint16_t FY_TYPE_COLORS[] = {
    ...
    0xF81F,              // FLOCK_WIFI_MOD  = will use WONTHOUND_HOTPINK at runtime
    0xFFFF,              // FLOCK_PIGVISION = will use WONTHOUND_BRIGHT at runtime
    0xF81F,              // FLOCK_MFR_MATCH = will use WONTHOUND_MAGENTA at runtime
};
...
static uint16_t fyTypeColor(uint8_t type) {
    switch (type) {
        ...
        case FLOCK_WIFI_MOD:   return WONTHOUND_HOTPINK;
        case FLOCK_PIGVISION:  return WONTHOUND_BRIGHT;
        case FLOCK_MFR_MATCH:  return WONTHOUND_MAGENTA;
...
static uint16_t fyProximityColor(int8_t rssi) {
    ...
    if (rssi >= -75) return WONTHOUND_HOTPINK; // HOTPINK — medium
    if (rssi >= -90) return WONTHOUND_MAGENTA; // MAGENTA — far
    return WONTHOUND_GUNMETAL;                 // GUNMETAL — weak
}
...
static void fyDrawRssiBar(int x, int y, int8_t rssi) {
    ...
    for (int s = 0; s < 6; s++) {
        uint16_t col = (s < segments) ? WONTHOUND_HOTPINK : WONTHOUND_GUNMETAL;
...
static void fyScanTask(void* param) {
    ...
    while (!fyScanStop) {
        BLEScanResults results;
        if (BLEScanResults* scanResultsPtr = pFyScan->start(FY_SCAN_DURATION, false)) {
            results = *scanResultsPtr;
        }
...
static void fyDrawIconBar() {
    ...
    tft.fillRect(0, 20, SCREEN_WIDTH, 16, WONTHOUND_GUNMETAL);
    // Back arrow (left)
    tft.drawBitmap(10, 20, bitmap_icon_go_back, FY_ICON_SIZE, FY_ICON_SIZE, WONTHOUND_MAGENTA);
    // BLE skull (center)
    tft.drawBitmap(112, 20, bitmap_icon_skull_bluetooth, FY_ICON_SIZE, FY_ICON_SIZE, WONTHOUND_HOTPINK);
    // Scanner/refresh (right)
    tft.drawBitmap(214, 20, bitmap_icon_scanner, FY_ICON_SIZE, FY_ICON_SIZE, WONTHOUND_MAGENTA);
    tft.drawLine(0, 36, SCREEN_WIDTH, 36, WONTHOUND_HOTPINK);
}
...
static void fyDrawStatus() {
    tft.fillRect(0, 57, SCREEN_WIDTH, 14, WONTHOUND_BLACK);
    ...
    if (!fyScanPaused && fyScanRunning) {
        ...
        if (dotOn) {
            tft.fillCircle(SCREEN_WIDTH - 20, 63, 3, WONTHOUND_HOTPINK);
        }
        tft.setTextColor(WONTHOUND_HOTPINK);
...
static void fyDrawStats() {
    tft.fillRect(0, 72, SCREEN_WIDTH, 18, WONTHOUND_BLACK);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    tft.drawLine(0, 89, SCREEN_WIDTH, 89, WONTHOUND_GUNMETAL);
}

static void fyDrawDeviceList() {
    // Clear list area (below stats divider, above buttons)
    tft.fillRect(0, 90, SCREEN_WIDTH, SCREEN_HEIGHT - 90 - 32, WONTHOUND_BLACK);
    ...
    for (int i = 0; i < visibleCount; i++) {
        ...
        if (idx == fySelectedIdx) {
            tft.fillRect(0, y - 1, SCREEN_WIDTH, FY_LINE_HEIGHT, WONTHOUND_MAGENTA);
        }
        ...
        // Type label
        uint16_t textCol = (idx == fySelectedIdx) ? WONTHOUND_BLACK : typeCol;
        tft.setTextSize(1);
        tft.setTextColor(textCol);
        tft.setCursor(8, y + 4);
        ...
        // MAC (last 4 octets)
        textCol = (idx == fySelectedIdx) ? WONTHOUND_BLACK : WONTHOUND_HOTPINK;
        tft.setTextColor(textCol);
        ...
        // RSSI
        textCol = (idx == fySelectedIdx) ? WONTHOUND_BLACK : WONTHOUND_VIOLET;
        tft.setTextColor(textCol);
        ...
        // Proximity label
        textCol = (idx == fySelectedIdx) ? WONTHOUND_BLACK : fyProximityColor(d.rssi);
        tft.setTextColor(textCol);
        ...
        if (d.seenCount <= 3) {
            uint16_t dotCol = (idx == fySelectedIdx) ? WONTHOUND_BLACK : WONTHOUND_HOTPINK;
...
static void fyDrawDetail() {
    tft.fillRect(0, 38, SCREEN_WIDTH, SCREEN_HEIGHT - 38, WONTHOUND_BLACK);
    ...
    tft.drawRect(10, y, 200, 8, WONTHOUND_VIOLET);
    y += 16;
    ...
    tft.drawRect(80, btnY, 80, 24, WONTHOUND_HOTPINK);
    tft.drawRect(81, btnY + 1, 78, 22, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void fyDrawBottomButtons() {
    ...
    // Clear button area
    tft.fillRect(0, btnY - 2, SCREEN_WIDTH, 30, WONTHOUND_BLACK);

    // STOP/START button (left)
    tft.drawRect(10, btnY, btnW, btnH, WONTHOUND_HOTPINK);
    tft.drawRect(11, btnY + 1, btnW - 2, btnH - 2, WONTHOUND_HOTPINK);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_HOTPINK);
    ...
    // SAVE button (center)
    tft.drawRect(90, btnY, btnW, btnH, WONTHOUND_HOTPINK);
    tft.drawRect(91, btnY + 1, btnW - 2, btnH - 2, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
    tft.setCursor(103, btnY + 8);
    tft.print("SAVE");

    // BACK button (right)
    tft.drawRect(170, btnY, btnW, btnH, WONTHOUND_HOTPINK);
    tft.drawRect(171, btnY + 1, btnW - 2, btnH - 2, WONTHOUND_HOTPINK);
    tft.setTextColor(WONTHOUND_HOTPINK);
...
static void fySaveLoot() {
    if (fyDeviceCount == 0) {
        fySetStatus("No devices to save!", WONTHOUND_GUNMETAL);
        delay(1000);
        return;
    }

    fySetStatus("Saving to SD...", WONTHOUND_MAGENTA);
    ...
    fySetStatus(statusBuf, WONTHOUND_GREEN);
...
static void fyHandleTouch() {
    ...
    if (ty >= 20 && ty <= 36) {
        ...
        if (tx >= 200 && tx <= 240) {
            ...
            fySetStatus(">> RESCANNING <<", WONTHOUND_HOTPINK);
            return;
        }
    }
