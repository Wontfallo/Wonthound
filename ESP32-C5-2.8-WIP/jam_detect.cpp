// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Jamming Detection Module
...
// Matches WontHound gradient bar style with magenta border
// ═══════════════════════════════════════════════════════════════════════════
...
static void drawCalibrationBar(int y, int percent) {
    ...
    tft.drawRect(barX - 1, y - 1, barW + 2, barH + 2, WONTHOUND_MAGENTA);
...
static void drawSkullMeter(int y, ThreatLevel level) {
    ...
    for (int i = 0; i < numSkulls; i++) {
        ...
        tft.fillRect(x, y, 16, 16, WONTHOUND_BLACK);
...
static void drawRateBar(int y, uint32_t rate, uint32_t baseline) {
    ...
    tft.drawRect(barX, y, barW, barH, elevated ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA);
    ...
    int tw = strlen(buf) * 6;
    tft.setTextColor(elevated ? TFT_WHITE : WONTHOUND_MAGENTA);
    tft.setTextSize(1);
...
void setup() {
    ...
    drawCenteredText(SCALE_Y(72), "Learning baseline...", WONTHOUND_MAGENTA, 1);
...
static void drawSpectrumBars() {
    ...
    for (int ch = 0; ch < FREQ_COUNT; ch++) {
        ...
        if (peakSeg > segs && peakSeg > 0) {
            ...
            tft.fillRect(x, peakY, SS_BAR_WIDTH, SS_SEG_H, WONTHOUND_HOTPINK);
        }
    }
}
...
void setup() {
    ...
    drawCenteredText(SCALE_Y(72), "Calibrating SubGHz...", WONTHOUND_MAGENTA, 1);
...
static void drawGwFrame() {
    // Y-axis
    tft.drawFastVLine(GW_BAR_X - 2, GW_BAR_Y, GW_BAR_H, WONTHOUND_MAGENTA);

    // X-axis
    tft.drawFastHLine(GW_BAR_X, GW_BAR_Y + GW_BAR_H, GW_BAR_W, WONTHOUND_MAGENTA);
    ...
    tft.drawFastHLine(0, GW_BAR_Y + GW_BAR_H + 16, SCREEN_WIDTH, WONTHOUND_HOTPINK);
}
...
static void drawGwBarGraph() {
    ...
    for (int i = 0; i < JD_NUM_SKULLS; i++) {
        ...
        tft.fillRect(sx, statusY, 16, 16, WONTHOUND_BLACK);

        if (i < litSkulls && peakLevel > 0) {
            tft.drawBitmap(sx, statusY, jdSkulls[i], 16, 16,
                skullWaveColor(jdSkullFrame, i));
        } else {
            tft.drawBitmap(sx, statusY, jdSkulls[i], 16, 16, WONTHOUND_GUNMETAL);
        }
    }
...
void setup() {
    ...
    if (!jdNrfInit()) {
        ...
        tft.setTextSize(1);
        drawCenteredText(130, "Check SPI wiring", WONTHOUND_MAGENTA, 1);
        delay(2000);
        exitRequested = true;
        initialized = true;
        return;
    ...
    drawCenteredText(SCALE_Y(72), "Calibrating 2.4GHz...", WONTHOUND_MAGENTA, 1);
...
static void drawBandIndicator(int y, const char* label, ThreatLevel level) {
    ...
    tft.drawCircle(dotX, dotY, 6, WONTHOUND_MAGENTA);
    if (level == THREAT_JAMMING) {
        tft.drawCircle(dotX, dotY, 7, WONTHOUND_HOTPINK);
    }
    ...
    else txtColor = WONTHOUND_HOTPINK;
...
static void drawMiniTimeline(int y, ThreatLevel* timeline) {
    ...
    tft.drawRect(startX - 1, y - 1, segW * TIMELINE_LEN + 2, barH + 2, WONTHOUND_MAGENTA);
    ...
    for (int i = 0; i < TIMELINE_LEN; i++) {
        ...
        switch (timeline[idx]) {
            ...
            default:                 color = WONTHOUND_GUNMETAL;  break;
        }
...
void setup() {
    ...
    drawCenteredText(SCALE_Y(72), "Calibrating all bands...", WONTHOUND_MAGENTA, 1);
