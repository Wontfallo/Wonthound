// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Touch Button Implementation
...
#include "icon.h"
#include "shared.h"
#if !defined(CYD_35) && !defined(CYD_CAP_TOUCH) && !defined(CYD_SHARED_XPT2046_TOUCH)
  ...
  #endif
#endif
#if defined(CYD_CAP_TOUCH) || defined(CYD_35) || defined(CYD_SHARED_XPT2046_TOUCH)
...
#ifdef CYD_FT6336_TOUCH
static FT6336 gt911(FT6336_SDA, FT6336_SCL, FT6336_INT, FT6336_RST,
                    CYD_SCREEN_WIDTH, CYD_SCREEN_HEIGHT);
static const char* CAP_TOUCH_NAME = "FT6336";
#else
static TAMC_GT911 gt911(GT911_SDA, GT911_SCL, GT911_INT, GT911_RST,
                        CYD_SCREEN_WIDTH, CYD_SCREEN_HEIGHT);
static const char* CAP_TOUCH_NAME = "GT911";
#endif
...
// Cap touch debounce: require consecutive reads to accept initial contact,
// then briefly ignore new touches after finger lifts (prevents bounce
// during release that resistive XPT2046 naturally avoids via pressure).
static uint8_t  _gt911HitCount = 0;
#ifdef CYD_FT6336_TOUCH
static const uint8_t  GT911_MIN_HITS = 1;
static const uint32_t GT911_RELEASE_COOLDOWN_MS = 45;
#else
...
// Normal reads are edge-triggered after consumeTouch(); peek reads are not.
static bool _getCachedTouch(uint16_t *x, uint16_t *y, bool ignoreConsumed = false) {
    uint32_t now = millis();
    if (now - _cacheTime >= 5) {
        _cacheTime = now;
    ...
    // Suppress normal reads after consumeTouch() until finger lifts.
    if (_touchFired && !ignoreConsumed) return false;
    if (_cacheValid) {
        *x = _cacheTX;
        *y = _cacheTY;
    }
...
#else // shared TFT_eSPI XPT2046 or capacitive touch
...
#endif // CYD_CAP_TOUCH
#endif // CYD_35 or capacitive touch
...
static bool isTopLeftBackCoords(uint16_t x, uint16_t y) {
#if defined(ESP32S3_ES3C28P) || defined(ESP32C5_NM_CYD)
    return x >= TOUCH_BTN_BACK_X1 && x <= TOUCH_BTN_BACK_X2 &&
           y >= TOUCH_BTN_BACK_Y1 && y <= TOUCH_BTN_BACK_Y2;
#else
    return x <= 52 &&
           y >= ICON_BAR_TOUCH_TOP && y <= ICON_BAR_TOUCH_BOTTOM;
#endif
}
...
void touchButtonsSetup() {
    ...
    #ifdef CYD_FT6336_TOUCH
      gt911.begin(FT6336_ADDR);
      gt911.setRotation(ROTATION_NORMAL);
    #else
      gt911.begin(GT911_ADDR1);
      gt911.setRotation(ROTATION_INVERTED);  // GT911 native coords match display
    #endif
    touch_calibrated = true;

    #if CYD_DEBUG
    Serial.printf("[TOUCH] %s capacitive touch init\n", CAP_TOUCH_NAME);
    Serial.flush();
    #endif
  #else
    ...
    #if CYD_DEBUG
    Serial.printf("[TOUCH] %s XPT2046 init (TFT_eSPI shared SPI, TOUCH_CS=%d)\n",
                  CYD_BOARD_NAME, TOUCH_CS);
    Serial.flush();
    #endif
...
void runTouchTest() {
    ...
    tft.setTextSize(2);
    tft.setCursor(10, 5);
#if defined(CYD_35) || defined(CYD_SHARED_XPT2046_TOUCH)
    ...
    while (digitalRead(BOOT_BUTTON) == HIGH) {
        ...
        if (gt911.isTouched && gt911.touches > 0) {
            ...
            tft.printf("%s X:%3d Y:%3d T:%d", CAP_TOUCH_NAME, screenX, screenY, gt911.touches);
            ...
            tft.printf("%s - no touch", CAP_TOUCH_NAME);
...
#ifdef CYD_FT6336_TOUCH
        // Let custom screen back handlers still see the global Back tap after
        // touchButtonsUpdate() has marked BTN_BACK for the frame.
        if (!_getCachedTouch(&tx, &ty, true) || !isTopLeftBackCoords(tx, ty)) return false;
#else
        return false;
#endif
    }
...
#if defined(CYD_CAP_TOUCH) || defined(CYD_35) || defined(CYD_SHARED_XPT2046_TOUCH)
    uint16_t tx, ty;
    if (!_getCachedTouch(&tx, &ty, true)) return -1;
...
void drawBackButton() {
    tft.drawBitmap(BACK_ICON_X, BACK_ICON_Y, bitmap_icon_go_back, BACK_ICON_SIZE, BACK_ICON_SIZE, WONTHOUND_MAGENTA);
}

bool isTopLeftBackTouch(uint16_t *x, uint16_t *y) {
#if defined(CYD_CAP_TOUCH) || defined(CYD_35) || defined(CYD_SHARED_XPT2046_TOUCH)
    uint16_t tx, ty;
    if (!_getCachedTouch(&tx, &ty, true)) return false;
    ...
    if (isTopLeftBackCoords((uint16_t)screenX, (uint16_t)screenY)) {
        if (x) *x = (uint16_t)screenX;
        if (y) *y = (uint16_t)screenY;
        return true;
    }
...
// Check if BACK button tapped - generous hitbox around the visible top-left icon
bool isBackButtonTapped() {
    static uint32_t lastTap = 0;

    if (millis() - lastTap < 100) return false;

    if (isTopLeftBackTouch()) {
        lastTap = millis();
        consumeTouch();  // One tap = one action
        return true;
    }
...
#ifdef CYD_FT6336_TOUCH
            // Keep Back visible to legacy custom icon-bar handlers in feature screens.
            if (touchedButton != BTN_BACK) consumeTouch();
#else
...
void runTouchCalibration() {
    ...
    tft.fillScreen(TFT_BLACK);
    tft.setTextColor(0xF81F);  // WONTHOUND_PINK
    tft.setTextSize(2);
    tft.setCursor(15, 80);
