// =============================================================================
// WontHound-CYD IoT Recon Module
...
static void updatePlagueAnimation() {
    ...
    for (int i = 0; i < 30; i++) {
        ...
        if (drawY >= SCALE_Y(125) && drawY < SCALE_Y(290)) {
            ...
            uint16_t charColor = col.infected ? WONTHOUND_HOTPINK : WONTHOUND_GREEN;
...
static void iotScanTask(void* param) {
    ...
    if (sdCredCount > 0) {
        ...
        addKillLine(msg, WONTHOUND_GREEN);
        newEventFlag = true;
    } else {
    ...
    }
    {
        ...
        addKillLine(msg, WONTHOUND_CYAN);
        newEventFlag = true;
    }
    ...
    addKillLine("[*] Scanning subnet...", WONTHOUND_CYAN);
    newEventFlag = true;
    ...
    for (int host = 1; host <= 254 && scanTaskRunning; host++) {
        ...
        if (portMask && deviceCount < IOT_MAX_DEVICES) {
            ...
            addKillLine(msg, WONTHOUND_MAGENTA);
            newEventFlag = true;
        }
        ...
        if (dev.type == IOT_CAMERA && (dev.openPorts & 0x01) && scanTaskRunning) {
            ...
            for (int c = 0; c < totalCredCount && scanTaskRunning && !cracked; c++) {
                ...
                if (c > 0 && c % 10 == 0) {
                    ...
                    addKillLine(msg, WONTHOUND_GUNMETAL);
                    newEventFlag = true;
                }
                ...
                if (client.connect(devIP, 554, 1000)) {
                    ...
                    if (resp.indexOf("200") >= 0) {
                        ...
                        addKillLine(msg, WONTHOUND_HOTPINK);
                        newEventFlag = true;
    ...
    addKillLine("=== SCAN COMPLETE ===", WONTHOUND_BRIGHT);
    if (hostsFound == 0) {
        addKillLine("[!] No devices found on subnet", WONTHOUND_HOTPINK);
        addKillLine("[!] Try a different network", WONTHOUND_GUNMETAL);
    } else {
        char summary[52];
        snprintf(summary, sizeof(summary), "[*] %d hosts, %d cracked, %d open",
                 (int)hostsFound, (int)camerasCracked, (int)openServices);
        addKillLine(summary, WONTHOUND_GREEN);
    }
    newEventFlag = true;
...
static void drawCredsScreen() {
    ...
    drawGlitchStatus(SCALE_Y(80), "HARVESTED", WONTHOUND_HOTPINK);
    ...
    if (capturedCredCount == 0) {
        ...
        for (int i = credScroll; i < capturedCredCount && (i - credScroll) < visibleLines; i++) {
            ...
            tft.fillRect(3, y, SCREEN_WIDTH - 6, lineH - 2, WONTHOUND_DARK);
            tft.drawRect(3, y, SCREEN_WIDTH - 6, lineH - 2, WONTHOUND_MAGENTA);
...
static void drawWifiScanScreen() {
    ...
    for (int i = networkScroll; i < networkCount && (i - networkScroll) < visibleLines; i++) {
        ...
        // Draw signal bars
        for (int b = 0; b < 4; b++) {
            uint16_t barColor = (b < bars) ? WONTHOUND_GREEN : WONTHOUND_GUNMETAL;
            tft.fillRect(5 + b * 4, y + (12 - (b + 1) * 3), 3, (b + 1) * 3, barColor);
        }

        // Lock icon or open
        tft.setTextColor(isOpen ? WONTHOUND_GREEN : WONTHOUND_MAGENTA);
...
static void drawKeyboardScreen(const char* prompt, int maxLen) {
    ...
    for (int row = 0; row < 4; row++) {
        ...
        for (int col = 0; col < (int)strlen(kbLayout[row]); col++) {
            tft.fillRect(xOffset, yOffset, kbKeyW, kbKeyH, WONTHOUND_DARK);
            tft.drawRect(xOffset, yOffset, kbKeyW, kbKeyH, WONTHOUND_GUNMETAL);
            tft.setTextColor(WONTHOUND_MAGENTA);
            tft.setTextSize(1);
    ...
    int clrX = SCALE_X(63);
    tft.fillRoundRect(clrX, btnY, btnW, btnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(clrX, btnY, btnW, btnH, 3, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    if (currentScreen == IOT_SCR_KEYBOARD_PASS) {
        ...
        tft.fillRoundRect(skipX, btnY, btnW, btnH, 3, WONTHOUND_DARK);
        tft.drawRoundRect(skipX, btnY, btnW, btnH, 3, WONTHOUND_GREEN);
        tft.setTextColor(WONTHOUND_GREEN);
    ...
    int okX = SCALE_X(179);
    tft.fillRoundRect(okX, btnY, btnW, btnH, 3, WONTHOUND_DARK);
    tft.drawRoundRect(okX, btnY, btnW, btnH, 3, WONTHOUND_MAGENTA);
    tft.setTextColor(WONTHOUND_MAGENTA);
...
static void handleKeyboardTouch(int x, int y, int maxLen) {
    ...
    for (int row = 0; row < 4; row++) {
        ...
        for (int col = 0; col < (int)strlen(kbLayout[row]); col++) {
            if (x >= xOffset && x <= xOffset + kbKeyW &&
                ...
                // Visual feedback
                tft.fillRect(xOffset, yOffset, kbKeyW, kbKeyH, WONTHOUND_HOTPINK);
                ...
                // Redraw input box
                int boxY = SCALE_Y(55);
                tft.fillRect(11, boxY + 1, SCREEN_WIDTH - 22, SCALE_H(25) - 2, WONTHOUND_DARK);
                tft.setTextColor(WONTHOUND_MAGENTA);
                tft.setCursor(15, boxY + 7);
                if (currentScreen == IOT_SCR_KEYBOARD_PASS) {
    ...
    if (x >= clrX && x <= clrX + btnW && y >= btnY && y <= btnY + btnH + 3) {
        ...
        tft.fillRect(11, boxY2 + 1, SCREEN_WIDTH - 22, SCALE_H(25) - 2, WONTHOUND_DARK);
        return;
    }
...
static void drawConnectingScreen() {
    ...
    drawGlitchStatus(SCALE_Y(120), "CONNECTING...", WONTHOUND_HOTPINK);
...
static void drawStatBadge(int x, int y, int w, int h, const char* label, int value, uint16_t borderColor) {
    tft.fillRoundRect(x, y, w, h, 2, WONTHOUND_DARK);
    ...
    tft.setTextSize(1);
    tft.setTextColor(borderColor, WONTHOUND_DARK);
...
static void drawStats() {
    int y = SCALE_Y(70);
    tft.fillRect(0, y, SCREEN_WIDTH, SCALE_H(50), WONTHOUND_BLACK);
    ...
    uint16_t borderColor = (scanPhase == IOT_PHASE_DONE) ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
    ...
    if (fillW < barW - 4) {
        tft.fillRect(barX + 2 + fillW, barY + 2, barW - 4 - fillW, barH - 4, WONTHOUND_DARK);
    }
    ...
    drawStatBadge(5,                          badgeY, badgeW, badgeH, "H",   (int)hostsFound,     WONTHOUND_CYAN);
    drawStatBadge(5 + (badgeW + gap),         badgeY, badgeW, badgeH, "Cam", (int)camerasFound,   WONTHOUND_MAGENTA);
    drawStatBadge(5 + (badgeW + gap) * 2,     badgeY, badgeW, badgeH, "Crk", (int)camerasCracked, WONTHOUND_HOTPINK);
    drawStatBadge(5 + (badgeW + gap) * 3,     badgeY, badgeW, badgeH, "Opn", (int)openServices,   WONTHOUND_GREEN);

    // ── STATS ROW 2: 3 badges, centered ──
    int row2Y = badgeY + badgeH + 2;
    int badgeW2 = (SCREEN_WIDTH - 10 - gap * 2) / 3;  // ~74px each

    drawStatBadge(5,                          row2Y, badgeW2, badgeH, "MQTT", (int)mqttFound,    WONTHOUND_CYAN);
    drawStatBadge(5 + (badgeW2 + gap),        row2Y, badgeW2, badgeH, "Tel",  (int)telnetFound,  WONTHOUND_VIOLET);
    drawStatBadge(5 + (badgeW2 + gap) * 2,    row2Y, badgeW2, badgeH, "Mod",  (int)modbusFound,  WONTHOUND_MAGENTA);
...
static void drawKillFeed() {
    ...
    // Clear feed area
    tft.fillRect(0, feedY, SCREEN_WIDTH, feedH, WONTHOUND_BLACK);
        ...
        case IOT_SCR_SCANNING:
        {
            ...
            if (scanDone && scanPhase == IOT_PHASE_DONE) {
                if (!autoSaved) {
                    ...
                    autoSaved = true;
                    if (sdReady) {
                        addKillLine("[SD] Report saved to /iot_recon.txt", WONTHOUND_GREEN);
                        killFeedDirty = true;
                    }
                }
            }
