// ═══════════════════════════════════════════════════════════════════════════
// WontHound-CYD Utility Functions Implementation
...
uint16_t WONTHOUND_MAGENTA = 0x041F;  // Electric Blue - Primary (selected items)
uint16_t WONTHOUND_HOTPINK = 0xF81F;  // Hot Pink - Accents
uint16_t WONTHOUND_BRIGHT  = 0xF81F;  // Hot Pink - Highlights
uint16_t WONTHOUND_VIOLET  = 0x780F;  // Purple - Accent color
uint16_t WONTHOUND_CYAN    = 0xF81F;  // Hot Pink for text (was cyan/blue)
uint16_t WONTHOUND_GREEN   = 0x780F;  // Purple (was neon green)
...
void drawTitleBar(const char* title) {
    ...
    tft.fillRect(0, STATUS_BAR_HEIGHT + 1, SCREEN_WIDTH, 25, WONTHOUND_DARK);
    ...
    // Draw separator line
    tft.drawLine(0, STATUS_BAR_HEIGHT + 26, SCREEN_WIDTH, STATUS_BAR_HEIGHT + 26, WONTHOUND_VIOLET);
}

void drawMenuItem(int y, const char* text, bool selected) {
    if (selected) {
        // Highlighted item
        tft.fillRect(0, y, SCREEN_WIDTH, 24, WONTHOUND_MAGENTA);
        tft.setTextColor(WONTHOUND_BLACK);
    } else {
        // Normal item
        tft.fillRect(0, y, SCREEN_WIDTH, 24, WONTHOUND_BLACK);
...
void drawProgressBar(int x, int y, int width, int height, int percent, uint16_t color) {
    ...
    // Draw border
    tft.drawRect(x, y, width, height, WONTHOUND_MAGENTA);
    ...
    if (fillWidth < width - 2) {
        tft.fillRect(x + 1 + fillWidth, y + 1, width - 2 - fillWidth, height - 2, WONTHOUND_BLACK);
    }
}
...
void drawGPSIndicator(int x, int y) {
    ...
    switch (status) {
        case GPS_NO_MODULE:
            ...
            break;
        case GPS_SEARCHING:
            color = WONTHOUND_HOTPINK;
            text = "GPS:..";
            break;
        case GPS_FIX_2D:
            color = WONTHOUND_BRIGHT;
            text = "GPS:2D";
            break;
        case GPS_FIX_3D:
            color = 0x07E0;  // Green
...
// Globals defined in WontHound-CYD.ino
...
void saveSettings() {
    ...
    settings.pinEnabled = 0;       // Gates disabled for this build
    settings.devicePin = 0;
    settings.disclaimerAccepted = 1;
    settings.blueTeamMode = 0;   // reserved (blue team removed)
...
void loadSettings() {
    ...
    if (settings.magic != EEPROM_MAGIC) {
        ...
        settings.displayInverted = 1;  // S3 IPS panel default
        #else
        ...
        settings.disclaimerAccepted = 1; // Gates disabled for this build
...
void applyColorOrder() {
    // MADCTL base values per rotation. TFT_eSPI setRotation() rewrites this
    // register, so keep board-specific values here instead of assuming ILI9341.
    // BGR bit is bit 3 (0x08) — clear it for RGB panels
    uint8_t madctl;
    uint8_t rot = tft.getRotation();
#if defined(ESP32C5_NM_CYD)
    // ST7789 on NM-CYD-C5. Do not set MX in portrait: it mirrors text.
    switch (rot) {
        case 0:  madctl = 0x08; break;  // BGR
        case 1:  madctl = 0x68; break;  // MX + MV + BGR
        case 2:  madctl = 0xC8; break;  // MX + MY + BGR
        case 3:  madctl = 0xA8; break;  // MY + MV + BGR
        default: madctl = 0x08; break;
    }
#else
    // ILI9341/ILI9341_2/ST7796 values used by the existing CYD/S3 display paths.
    switch (rot) {
...
void applyColorMode(uint8_t mode) {
    switch (mode) {
        case 1: // Deuteranopia — blue + yellow (red-green colorblind safe)
            WONTHOUND_MAGENTA = 0x067F;  // Bright Blue — primary selection
            WONTHOUND_HOTPINK = 0xFFE0;  // Yellow — accents
            WONTHOUND_BRIGHT  = 0xFFE0;  // Yellow — highlights
            WONTHOUND_VIOLET  = 0x067F;  // Bright Blue — accent
            WONTHOUND_CYAN    = 0xFFE0;  // Yellow — text
            WONTHOUND_GREEN   = 0x067F;  // Bright Blue — secondary
            break;
        case 2: // High Contrast — white + cyan (maximum visibility)
            WONTHOUND_MAGENTA = 0x07FF;  // Bright Cyan — primary selection
            WONTHOUND_HOTPINK = 0xFFFF;  // White — accents
            WONTHOUND_BRIGHT  = 0xFFFF;  // White — highlights
            WONTHOUND_VIOLET  = 0x07FF;  // Bright Cyan — accent
            WONTHOUND_CYAN    = 0xFFFF;  // White — text
            WONTHOUND_GREEN   = 0x07E0;  // True Green — secondary
            break;
        default: // Mode 0 — Default (Jesse's pink/purple theme)
            WONTHOUND_MAGENTA = 0x041F;  // Electric Blue
            WONTHOUND_HOTPINK = 0xF81F;  // Hot Pink
            WONTHOUND_BRIGHT  = 0xF81F;  // Hot Pink
            WONTHOUND_VIOLET  = 0x780F;  // Purple
            WONTHOUND_CYAN    = 0xF81F;  // Hot Pink
            WONTHOUND_GREEN   = 0x780F;  // Purple
            break;
    }
...
void printSystemInfo() {
    #if CYD_DEBUG
    Serial.println("═══════════════════════════════════════════════════════════");
    Serial.println("              WONTHOUND-CYD SYSTEM INFO");
    Serial.println("═══════════════════════════════════════════════════════════");
