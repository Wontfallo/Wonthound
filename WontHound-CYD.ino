...
// Legacy accent colors kept for the disclaimer screen drawing code.
...
int last_main_menu_page = -1;
int submenu_page = 0;
int last_submenu_page = -1;
...
const int MAIN_MENU_NEXT_PAGE = -1;
const int MAIN_MENU_PREV_PAGE = -2;
const int MAIN_MENU_PAGE_COUNT = 2;
const int main_menu_page0[] = {0, 1, 2, 5, 7, MAIN_MENU_NEXT_PAGE};
const int main_menu_page1[] = {6, 8, 9, MAIN_MENU_PREV_PAGE};
const int *main_menu_pages[MAIN_MENU_PAGE_COUNT] = {main_menu_page0, main_menu_page1};
const int main_menu_page_sizes[MAIN_MENU_PAGE_COUNT] = {6, 4};

static const char* mainMenuLabelForSlot(int slot) {
    if (slot == MAIN_MENU_NEXT_PAGE) return "More...";
    if (slot == MAIN_MENU_PREV_PAGE) return "Back";
    return menu_items[slot];
}

static const unsigned char* mainMenuIconForSlot(int slot) {
    if (slot == MAIN_MENU_NEXT_PAGE) return bitmap_icon_go_back;
    if (slot == MAIN_MENU_PREV_PAGE) return bitmap_icon_go_back;
    return bitmap_icons[slot];
}

const int SUBMENU_NAV_PREV = -1;
const int SUBMENU_NAV_NEXT = -2;
const int SUBMENU_ROWS_PER_PAGE = 6;
const int SUBMENU_REAL_ITEMS_PER_PAGE = 5;
...
static int buildSubmenuVisibleSlots(int slots[SUBMENU_ROWS_PER_PAGE]) {
    if (!active_submenu_items || active_submenu_size <= 0) return 0;

    if (active_submenu_size <= SUBMENU_ROWS_PER_PAGE) {
        for (int i = 0; i < active_submenu_size; i++) slots[i] = i;
        return active_submenu_size;
    }

    int count = 0;
    int start = submenu_page * SUBMENU_REAL_ITEMS_PER_PAGE;
    if (start >= active_submenu_size) {
        submenu_page = 0;
        start = 0;
    }

    if (submenu_page > 0) {
        slots[count++] = SUBMENU_NAV_PREV;
    }

    int roomForItems = SUBMENU_ROWS_PER_PAGE - count;
    bool hasMore = (start + roomForItems) < active_submenu_size;
    if (hasMore) roomForItems--;

    for (int i = 0; i < roomForItems && (start + i) < active_submenu_size; i++) {
        slots[count++] = start + i;
    }

    if (hasMore && count < SUBMENU_ROWS_PER_PAGE) {
        slots[count++] = SUBMENU_NAV_NEXT;
    }

    return count;
}

static const char* submenuLabelForSlot(int slot) {
    if (slot == SUBMENU_NAV_PREV) return "Back Page";
    if (slot == SUBMENU_NAV_NEXT) return "More...";
    return active_submenu_items[slot];
}

static const unsigned char* submenuIconForSlot(int slot) {
    if (slot == SUBMENU_NAV_PREV || slot == SUBMENU_NAV_NEXT) return bitmap_icon_go_back;
    return active_submenu_icons[slot];
}

static int submenuRowTop() {
    return CONTENT_Y_START + 8;
}

static int submenuRowHeight(int visibleCount) {
    int h = (SCREEN_HEIGHT - 8 - submenuRowTop()) / (visibleCount > 0 ? visibleCount : 1);
    if (h < 38) h = 38;
    return h;
}

static int getTouchedSubmenuIndex() {
    int slots[SUBMENU_ROWS_PER_PAGE];
    int visibleCount = buildSubmenuVisibleSlots(slots);
    int rowTop = submenuRowTop();
    int rowH = submenuRowHeight(visibleCount);

    for (int row = 0; row < visibleCount; row++) {
        int y = rowTop + row * rowH;
        if (isTouchInArea(6, y, SCREEN_WIDTH - 12, rowH)) {
            int slot = slots[row];
            if (slot == SUBMENU_NAV_PREV) {
                if (submenu_page > 0) submenu_page--;
                submenu_initialized = false;
                displaySubmenu();
                delay(120);
                return -1;
            }
            if (slot == SUBMENU_NAV_NEXT) {
                submenu_page++;
                submenu_initialized = false;
                displaySubmenu();
                delay(120);
...
// Liability disclaimer state. Blue-team lockout is disabled.
bool disclaimer_accepted = false;   // Legal disclaimer accepted (persisted in EEPROM)
bool blue_team_mode = false;        // Legacy flag; forced off at load/save
...
void displaySubmenu() {
    ...
    if (!submenu_initialized || last_submenu_index != current_submenu_index || last_submenu_page != submenu_page) {
        tft.fillScreen(TFT_BLACK);

        tft.fillRect(0, 0, SCREEN_WIDTH, CONTENT_Y_START, WONTHOUND_DARK);
        tft.setTextFont(1);
        tft.setTextSize(2);
        ...
        tft.setCursor(SCREEN_WIDTH - 58, 14);
        int totalPages = (active_submenu_size <= SUBMENU_ROWS_PER_PAGE) ? 1 :
                         ((active_submenu_size + SUBMENU_REAL_ITEMS_PER_PAGE - 1) / SUBMENU_REAL_ITEMS_PER_PAGE);
        tft.printf("%d/%d", submenu_page + 1, totalPages);
        tft.drawBitmap(SCREEN_WIDTH - 26, 20, bitmap_icon_go_back, 16, 16, WONTHOUND_MAGENTA);
        tft.drawLine(0, CONTENT_Y_START - 1, SCREEN_WIDTH, CONTENT_Y_START - 1, WONTHOUND_MAGENTA);
        ...
        for (int row = 0; row < visibleCount; row++) {
            int slot = slots[row];
            int yPos = rowTop + row * rowH;
            bool selected = (slot >= 0 && slot == current_submenu_index);
            uint16_t bg = selected ? WONTHOUND_DARK : TFT_BLACK;
            uint16_t fg = selected ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;

            tft.fillRect(6, yPos, SCREEN_WIDTH - 12, rowH - 3, bg);
            tft.drawFastHLine(8, yPos + rowH - 3, SCREEN_WIDTH - 16, selected ? WONTHOUND_HOTPINK : 0x18C3);
            if (selected) {
                tft.drawRect(6, yPos, SCREEN_WIDTH - 12, rowH - 3, WONTHOUND_HOTPINK);
                tft.fillRect(6, yPos, 3, rowH - 3, WONTHOUND_HOTPINK);
            }

            int iconY = yPos + (rowH - 16) / 2;
            int textY = yPos + (rowH - 16) / 2;
            tft.drawBitmap(14, iconY, submenuIconForSlot(slot), 16, 16, fg);
            tft.setTextSize(2);
            tft.setTextColor(fg, bg);
            tft.setCursor(38, textY);
            tft.print(submenuLabelForSlot(slot));
        }

        submenu_initialized = true;
        last_submenu_index = current_submenu_index;
        last_submenu_page = submenu_page;
    }
...
// ═══════════════════════════════════════════════════════════════════════════
// DISPLAY MAIN MENU - CLEAR LIST VIEW
// ═══════════════════════════════════════════════════════════════════════════
...
void displayMenu() {
    ...
    if (!menu_initialized || last_menu_index != current_menu_index || last_main_menu_page != main_menu_page) {
        tft.fillScreen(TFT_BLACK);
        ...
        tft.setTextSize(1);
        tft.setTextColor(WONTHOUND_GUNMETAL);
        tft.setCursor(SCREEN_WIDTH - 34, 14);
        tft.printf("%d/%d", main_menu_page + 1, MAIN_MENU_PAGE_COUNT);
        tft.drawLine(0, CONTENT_Y_START - 1, SCREEN_WIDTH, CONTENT_Y_START - 1, WONTHOUND_MAGENTA);

        const int *pageItems = main_menu_pages[main_menu_page];
        int pageSize = main_menu_page_sizes[main_menu_page];
        int menuTop = CONTENT_Y_START + 8;
        int menuBottom = SCREEN_HEIGHT - 6;
        int rowH = (menuBottom - menuTop) / pageSize;
        if (rowH < 36) rowH = 36;

        for (int i = 0; i < pageSize; i++) {
            int slot = pageItems[i];
            int y = menuTop + i * rowH;
            bool selected = (slot >= 0 && slot == current_menu_index);
            uint16_t bg = selected ? WONTHOUND_DARK : TFT_BLACK;
            uint16_t fg = selected ? WONTHOUND_HOTPINK : WONTHOUND_MAGENTA;
            uint16_t line = selected ? WONTHOUND_HOTPINK : 0x18C3;

            tft.fillRect(6, y, SCREEN_WIDTH - 12, rowH - 2, bg);
            tft.drawFastHLine(8, y + rowH - 2, SCREEN_WIDTH - 16, line);
            if (selected) {
                tft.drawRect(6, y, SCREEN_WIDTH - 12, rowH - 2, WONTHOUND_HOTPINK);
                tft.fillRect(6, y, 3, rowH - 2, WONTHOUND_HOTPINK);
            }

            int iconY = y + (rowH - 16) / 2;
            tft.drawBitmap(16, iconY, mainMenuIconForSlot(slot), 16, 16, fg);
            tft.setTextColor(fg, bg);
            tft.setTextSize(2);
            tft.setCursor(42, y + (rowH - 16) / 2);
            tft.print(mainMenuLabelForSlot(slot));
        }

        menu_initialized = true;
        last_menu_index = current_menu_index;
        last_main_menu_page = main_menu_page;
    }

    // Lock icon — top-right, below battery indicator, only when PIN is enabled
    if (pin_enabled) {
        tft.drawBitmap(SCREEN_WIDTH - 20, 14, bitmap_icon_eye2, 16, 16, WONTHOUND_HOTPINK);
    }
...
void handleWiFiSubmenuTouch() {
    ...
    for (int i = touchedIndex; i <= touchedIndex; i++) {
...
void displayDeviceInfo() {
    ...
    y += 18;
    tft.setCursor(10, y); tft.print("By: Wontfallo");
    y += 18;
...
// ═══════════════════════════════════════════════════════════════════════════
// ABOUT PAGE HANDLER
// Full-screen about page — clean title, module list, and hardware stats
// ═══════════════════════════════════════════════════════════════════════════
...
void handleAboutPage() {
    ...
    drawCenteredText(my, "By: Wontfallo", WONTHOUND_MAGENTA, 1);
    my += 14;
    drawCenteredText(my, "github.com/wontfallo", WONTHOUND_VIOLET, 1);

    drawCenteredText(SCREEN_HEIGHT - 18, "WontHound-CYD", WONTHOUND_GUNMETAL, 1);
...
// ═══════════════════════════════════════════════════════════════════════════
// Legal disclaimer. Blue-team lockout has been removed.
// ═══════════════════════════════════════════════════════════════════════════
...
// Legacy no-op: blue-team lockout has been removed.
void showBlueTeamBlockedScreen() {
    ...
    drawGlitchStatus(SCALE_Y(125), "MODE ACTIVE", WONTHOUND_VIOLET);

    // Info text
    drawCenteredText(SCALE_Y(160), "Offensive functions disabled", WONTHOUND_MAGENTA, 1);
    drawCenteredText(SCALE_Y(180), "VALHALLA protocol engaged", WONTHOUND_GUNMETAL, 1);

    // Hint to unlock
    drawCenteredText(SCALE_Y(220), "Tap offensive tool to unlock", WONTHOUND_GUNMETAL, 1);
    drawCenteredText(SCALE_Y(235), "via disclaimer acceptance", WONTHOUND_GUNMETAL, 1);
...
// Legacy no-op: blue-team/VALHALLA activation has been removed.
void activateValhalla() {
    ...
    drawGlitchStatus(SCALE_Y(195), "PROTOCOL", WONTHOUND_VIOLET);

    // Separator
    tft.drawLine(20, SCALE_Y(207), SCREEN_WIDTH - 20, SCALE_Y(207), TFT_RED);

    // Warning text
    drawCenteredText(SCALE_Y(215), "ACTIVATE SCORCHED EARTH?", WONTHOUND_HOTPINK, 1);
    tft.setTextSize(1);
    tft.setTextColor(WONTHOUND_MAGENTA);
    ...
    // Separator
    tft.drawLine(20, SCALE_Y(268), SCREEN_WIDTH - 20, SCALE_Y(268), WONTHOUND_VIOLET);
    ...
    tft.fillRoundRect(confirmX, confirmY, confirmW, confirmH, 5, WONTHOUND_DARK);
    ...
    tft.fillRoundRect(cancelX, cancelY, cancelW, cancelH, 5, WONTHOUND_DARK);
    tft.drawRoundRect(cancelX, cancelY, cancelW, cancelH, 5, WONTHOUND_GUNMETAL);
    tft.setTextColor(WONTHOUND_GUNMETAL);
...
void handleButtons() {
    ...
    if (in_sub_menu) {
        ...
        for (int i = 0; i < pageSize; i++) {
            int slot = pageItems[i];
            int y_position = menuTop + i * rowH;

            if (isTouchInArea(6, y_position, SCREEN_WIDTH - 12, rowH)) {
                last_interaction_time = millis();
                if (slot == MAIN_MENU_NEXT_PAGE) {
                    main_menu_page = 1;
                    menu_initialized = false;
                    displayMenu();
                    delay(120);
...
void showSplash() {
    ...
    drawGlitchStatus(170, FW_EDITION, WONTHOUND_MAGENTA);

    // Version
    tft.setTextSize(1);
    drawCenteredText(200, FW_VERSION, WONTHOUND_HOTPINK, 1);

    // Board info
    drawCenteredText(215, CYD_BOARD_NAME, WONTHOUND_HOTPINK, 1);
#else
    // Title — glitch effect
    drawGlitchTitle(80, "WONTHOUND");

    // Subtitle — shows CYD or CYD-HAT so user knows which firmware they flashed
    drawGlitchStatus(110, FW_EDITION, WONTHOUND_MAGENTA);

    // Version
    tft.setTextSize(1);
    drawCenteredText(130, FW_VERSION, WONTHOUND_HOTPINK, 1);

    // Board info
    drawCenteredText(140, CYD_BOARD_NAME, WONTHOUND_HOTPINK, 1);
#endif

    // Credits
    drawCenteredText(SCREEN_HEIGHT - 40, "by Wontfallo", WONTHOUND_VIOLET, 1);
...
#if defined(ESP32S3_ES3C28P)
    Serial.printf("[INIT] PSRAM: %s, free=%u\n", psramFound() ? "OK" : "missing", ESP.getFreePsram());
    if (storageBegin()) {
