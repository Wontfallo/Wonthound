...
#else
bool display_inverted = true;  // this CYD ILI9341 panel needs inversion ON (else colors look like a photo negative)
#endif
...
// ── Fixed single-screen "dashboard" home (classic CYD / no-PSRAM variant) ──
// All 10 menu items live at static positions: 7 functional icons arranged around
// the central Anonymous bulb + 3 utility icons (Tools/Setting/About) in a bottom
// bar. Drawn ONCE and never scrolled, so there is zero flicker on the PSRAM-less
// classic ESP32, and every tap is a clean fixed hit-test rect (ideal for the
// resistive panel — no constant-pressure dragging). The S3 keeps its PSRAM
// sprite-scroll home; this is the 3E layout that matches the UI concept mockup.
...
// No sprite / no scrolling = no flicker, even without PSRAM. Icons are drawn with the
// transparent color-key over the Anonymous-bulb background.
void drawMenuFrame() {
...
bool isOffensiveAllowed() {
    return true;   // disclaimer screen removed — tools always allowed
}

// ─── Legal Disclaimer Screen ────────────────────────────────────────────
// Disabled per user request: never shown. Always reports "accepted" so the
// per-tool gates (`if (!showDisclaimerScreen()) break;`) pass straight through.
bool showDisclaimerScreen() {
    disclaimer_accepted = true;
    return true;
}

// Original disclaimer UI kept below but no longer reached.
bool showDisclaimerScreen_unused() {
...
#else
    tft.invertDisplay(true);   // this CYD ILI9341 panel needs inversion ON (else colors look like negative film)
#endif
    tft.fillScreen(WONTHOUND_BLACK);
